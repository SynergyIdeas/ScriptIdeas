...
try {
    ...
    # Critical: Cache drive removal prompt before Platform Layer finalization
    Write-Host "=" * 80 -ForegroundColor Red
    Write-Host "IMPORTANT: CACHE DRIVE REMOVAL REQUIRED" -ForegroundColor Red
    Write-Host "=" * 80 -ForegroundColor Red
    Write-Host ""
    Write-Host "Before proceeding with Platform Layer finalization:" -ForegroundColor Yellow
    Write-Host "1. REMOVE the D: cache drive from the virtual machine" -ForegroundColor Yellow
    Write-Host "2. The cache drive contains temporary installation files and logs" -ForegroundColor Yellow
    Write-Host "3. Platform Layer finalization requires the cache drive to be detached" -ForegroundColor Yellow
    Write-Host "4. After removal, the system will be ready for Platform Layer sealing" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Steps to remove cache drive:" -ForegroundColor Cyan
    Write-Host "- Power off the virtual machine" -ForegroundColor Cyan
    Write-Host "- Remove the D: drive from VM hardware configuration" -ForegroundColor Cyan
    Write-Host "- Power on the VM to complete Platform Layer finalization" -ForegroundColor Cyan
    Write-Host ""
