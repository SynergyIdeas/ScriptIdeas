            ...
            if ($WEMProcess.ExitCode -eq 0) {
                ...
                if ($WEMService) {
                    ...
                    if ($ConfigureWEMAgentCache) {
                        $WEMAgentCacheLocation = Get-ConfigValue -Key "WEMAgentCacheLocation" -DefaultValue "D:\" -ConfigFile $ConfigFilePath
                        
                        # Validate cache location is not empty
                        if ([string]::IsNullOrWhiteSpace($WEMAgentCacheLocation)) {
                            Write-Log "WEM Agent cache location is empty - using default: D:\" "WARN"
                            $WEMAgentCacheLocation = "D:\"
                        }
                        
                        # Ensure we have a valid, non-empty string before proceeding
                        if ([string]::IsNullOrWhiteSpace($WEMAgentCacheLocation) -or $WEMAgentCacheLocation.Trim() -eq "") {
                            Write-Log "WEM Agent cache location validation failed - skipping configuration" "ERROR"
                            $Results.CacheLocationConfigured = $false
                        } else {
                            # Trim whitespace and ensure it's a valid path
                            $WEMAgentCacheLocation = $WEMAgentCacheLocation.Trim()
