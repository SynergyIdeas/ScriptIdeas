...
function New-VirtualCacheDrive {
    ...
    try {
        ...
        try {
            # Cross-platform virtual drive creation with multiple fallback methods
            $VHDXCreated = $false
            
            # Method 1: Try Hyper-V PowerShell cmdlets first
            if (Get-Command "New-VHD" -ErrorAction SilentlyContinue) {
                try {
                    # Ensure minimum size requirements (New-VHD requires at least 3MB)
                    $SizeInBytes = [int64]$CacheDriveSizeMB * 1MB
                    $MinimumSize = 3MB
                    
                    if ($SizeInBytes -lt $MinimumSize) {
                        Write-Log "Adjusting VHDX size from ${CacheDriveSizeMB}MB to minimum 3MB" "WARN"
                        $SizeInBytes = $MinimumSize
                        $CacheDriveSizeMB = 3
                    }
                    
                    Write-Log "Creating VHDX with size: $([Math]::Round($SizeInBytes/1MB,2))MB ($SizeInBytes bytes)" "INFO"
                    
                    # Try SizeBytes parameter first (newer versions)
                    try {
                        New-VHD -Path $CacheVHDXPath -SizeBytes $SizeInBytes -Dynamic -ErrorAction Stop
                        Write-Log "New-VHD succeeded with -SizeBytes parameter" "SUCCESS"
                    } catch {
                        Write-Log "SizeBytes parameter failed: $($_.Exception.Message)" "WARN"
                        # Fallback to Size parameter (older versions)
                        New-VHD -Path $CacheVHDXPath -Size $SizeInBytes -Dynamic -ErrorAction Stop
                        Write-Log "New-VHD succeeded with -Size parameter" "SUCCESS"
                    }
                    
                    Mount-VHD -Path $CacheVHDXPath
                    $VHDDisk = Get-VHD -Path $CacheVHDXPath | Get-Disk
                    Initialize-Disk -Number $VHDDisk.Number -PartitionStyle MBR
                    $Partition = New-Partition -DiskNumber $VHDDisk.Number -UseMaximumSize -DriveLetter $DriveLetter
                    Format-Volume -DriveLetter $Partition.DriveLetter -FileSystem NTFS -NewFileSystemLabel $VolumeLabel -Force
                    $VHDXCreated = $true
                    Write-Log "VHDX created successfully using Hyper-V cmdlets" "SUCCESS"
                } catch {
                    Write-Log "Hyper-V method failed: $($_.Exception.Message)" "WARN"
                }
            }
            
            # Method 2: Try diskpart if Hyper-V failed and diskpart is available
            if (-not $VHDXCreated -and (Get-Command "diskpart" -ErrorAction SilentlyContinue)) {
                try {
                    $DiskpartScript = @"
