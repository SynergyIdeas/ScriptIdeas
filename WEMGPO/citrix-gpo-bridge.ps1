                "/api/get-ad-group-policies" {
                    # Get parameters from query string
                    $Domain = $Request.QueryString["domain"]
                    
                    # Call the function to get AD Group Policies
                    $Policies = Get-ADGroupPolicies -Domain $Domain
                ...
                "/api/export-ad-gpo" {
                    # Get parameters from query string
                    $GPOId = $Request.QueryString["gpoId"]
                    $Domain = $Request.QueryString["domain"]
                    $OutputPath = $Request.QueryString["outputPath"]
                    
                    # Call the export function
                    $Result = Export-ADGPOToXML -GPOId $GPOId -OutputPath $OutputPath -Domain $Domain
                    
                    # Return result as JSON
                    $ResultJson = @{
                        "success" = ($Result -ne $null)
                        "path" = $Result
                        "overwritten" = (Test-Path -Path $OutputPath -PathType Leaf -ErrorAction SilentlyContinue)
            ...
            switch ($Request.Url.LocalPath) {
                ...
                "/api/compare" {
                    ...
                    $ResultJson = @{
                        ...
                        "result" = $Result
                        "path" = $OutputXmlPath
                        "overwritten" = (Test-Path -Path $OutputXmlPath -PathType Leaf -ErrorAction SilentlyContinue)
                        "error" = if ($Result -eq $null) { $Error[0].ToString() } else { $null }
