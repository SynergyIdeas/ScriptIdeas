    ...
    if ($Global:CachedConfig.InstallUberAgent) {
        ...
        try {
            ...
            if ($UberAgentService) {
                ...
                Write-Log "UberAgent service not found - may not be installed" "WARN"
                $UberAgentServiceResult = @{ 
                    Success = $false
                    Error = "Service not found"
                    Details = "UberAgent service not installed or not accessible"
                }
            }
        }
        catch {
        ...
        Write-Log "UberAgent service configuration skipped - UberAgent not installed" "INFO"
        $UberAgentServiceResult = @{ 
            Success = $false; 
            Skipped = $true; 
            Message = "UberAgent service configuration skipped - UberAgent not installed";
            Details = "UberAgent installation disabled in configuration"
        }
    }
