...
function Remove-ActiveComponentsRegistry {
    <#
    .SYNOPSIS
        Cleans up Active Components registry keys
    .DESCRIPTION
        Removes Active Components registry entries that can cause VDI performance issues
    .EXAMPLE
            ...
            if (Test-Path $BasePath) {
                ...
                try {
                    ...
                    foreach ($Component in $Components) {
                        $ComponentName = $Component.PSChildName
                        $ComponentPath = $Component.PSPath
                        
                        # Skip critical Windows components
                        $SkipComponents = @(
                            "{89820200-ECBD-11cf-8B85-00AA005B4340}",  # Windows Script Host
                            "{89820200-ECBD-11cf-8B85-00AA005B4383}",  # IE Core Fonts
                            ...
                            "{2C7339CF-2B09-4501-B3F3-F3508C9228ED}",  # Windows Defender
                            "{5fd399c0-a70a-11d1-9948-00c04f98bbc9}",  # Internet Explorer Setup
                            "{89820200-ECBD-11cf-8B85-00AA005B4340}",  # Script engines
                            "{7790769C-0471-11d2-AF11-00C04FA35D02}"   # Address book
                        )
                        
                        if ($ComponentName -notin $SkipComponents) {
                            try {
                                # Get component properties to determine if it should be removed
                                $ComponentProperties = Get-ItemProperty -Path $ComponentPath -ErrorAction SilentlyContinue
                                
                                # Remove if it has StubPath OR if it's a non-critical component
                                $ShouldRemove = $false
                                
                                # Check for StubPath (runs on logon)
                                if ($ComponentProperties.StubPath) {
                                    $ShouldRemove = $true
                                    Write-Log "Component has StubPath: $ComponentName" "INFO"
                                }
                                
                                # Check for Version property (indicates installed component)
                                elseif ($ComponentProperties.Version) {
                                    $ShouldRemove = $true
                                    Write-Log "Component has Version info: $ComponentName" "INFO"
                                }
                                
                                # Remove third-party components (non-Microsoft GUIDs)
                                elseif ($ComponentName -notmatch "^\{[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}\}$") {
                                    $ShouldRemove = $true
                                    Write-Log "Non-GUID component: $ComponentName" "INFO"
                                }
                                
                                if ($ShouldRemove) {
                                    Write-Log "Removing Active Component: $ComponentName" "INFO"
                                    Remove-Item -Path $ComponentPath -Recurse -Force -ErrorAction Stop
                                    $RemovedComponents += $ComponentName
                                    $TotalRemoved++
                                }
                                else {
                                    Write-Log "Skipping component (no removal criteria met): $ComponentName" "INFO"
                                }
                            }
                            catch {
                                Write-Log "Failed to remove component $ComponentName`: $($_.Exception.Message)" "WARN"
                            }
                        }
                        else {
                            Write-Log "Skipping critical component: $ComponentName" "INFO"
                        }
                    }
                }
                catch {
