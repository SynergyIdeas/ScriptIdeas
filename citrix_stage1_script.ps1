...
#region Configuration Section - Modify paths as needed
# =============================================================================
# INSTALLATION PATHS CONFIGURATION
# =============================================================================

# Network source path for installation files (modify as needed)
$NetworkSourcePath = "\\server\share\CitrixInstallers"   # Network location with installer files

# Local installation paths (files will be copied to C:\Temp)
...
# UberAgent paths (set to empty string to skip UberAgent installation)
$UberAgentPath = "C:\Temp\uberAgent_7.1.2_x64.msi"
$UberAgentTemplatesPath = "C:\Temp\UberAgent\Templates"
$UberAgentConfigPath = "C:\Temp\UberAgent\uberagent.conf"
$UberAgentLicensePath = "C:\Temp\UberAgent\uberagent.license"

# IBM TADDM paths (set to empty string to skip TADDM configuration)
$TADDMPath = "C:\Temp\7.3.0.6-TIV-TADDM-Windows.exe"
$TADDMInstallBatPath = ""                                 # Auto-detect install.bat

# Script and logging configuration
$FunctionsPath = "$PSScriptRoot\citrix_functions_library.psm1" # Functions module path
$LogPath = "C:\Logs\CitrixInstall.log"                   # Main log file
$Stage2ScriptPath = "$PSScriptRoot\citrix_stage2_script.ps1"   # Stage 2 script location

# Drive configuration settings
$PagefileSizeGB = 8                                       # Fixed pagefile size in GB (8GB recommended for VDI)
...
Write-Host "VDA: $VDAPath"
Write-Host "PVS: $(if([string]::IsNullOrEmpty($PVSPath)){'SKIP'}else{$PVSPath})"
...
if (Test-Path $VDAPath) {
    Write-Host "  VDA installer: FOUND" -ForegroundColor Green
    
    # Enhanced VDA validation
    try {
        $VDAFileInfo = Get-ItemProperty -Path $VDAPath
        $VDASize = [Math]::Round($VDAFileInfo.Length / 1MB, 1)
        Write-Host "  VDA installer size: $VDASize MB" -ForegroundColor Gray
        
        if ($VDASize -lt 100) {
            $ValidationWarnings += "VDA installer seems unusually small ($VDASize MB)"
        }
    }
    catch {
        $ValidationWarnings += "Could not validate VDA installer properties"
    }
}
else {
    $ValidationErrors += "VDA installer not found: $VDAPath"
    Write-Host "  VDA installer: NOT FOUND" -ForegroundColor Red
}
...
$OptionalComponents = @(
    @{ Name = "PVS Target Device"; Path = $PVSPath; Variable = "PVSPath" },
...
Write-Host "  VDA: $VDAPath"
if (![string]::IsNullOrEmpty($PVSPath)) { Write-Host "  PVS: $PVSPath" }
...
try {
    ...
    $CopySuccess = Copy-InstallationFiles -NetworkPath $NetworkSourcePath -LocalPath "C:\Temp" -Force
    ...
    Write-Log "VDA Path: $VDAPath"
    Write-Log "PVS Path: $(if([string]::IsNullOrEmpty($PVSPath)){'Not specified - SKIP'}else{$PVSPath})"
    ...
    $InstallConfig.VDAPath = $VDAPath
    $InstallConfig.PVSPath = $PVSPath
    ...
    # Install VDA (no delivery controller required)
    Write-Log "Installing Citrix VDA (no delivery controller required)..."
    $VDAResult = Install-CitrixVDA -VDAPath $VDAPath -LogDir (Split-Path $LogPath -Parent)
    ...
    # Install PVS Target Device (if specified, no PVS server required)
    if (![string]::IsNullOrEmpty($PVSPath)) {
        Write-Log "Installing PVS Target Device (no PVS server required)..."
        $PVSResult = Install-PVSTargetDevice -PVSPath $PVSPath
