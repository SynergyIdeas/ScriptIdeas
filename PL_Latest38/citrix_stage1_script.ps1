...
try {
    ...
    try {
        # Ensure the config file path is absolute and exists
        if (-not [System.IO.Path]::IsPathRooted($ConfigFilePath)) {
            $ConfigFilePath = Join-Path $PSScriptRoot "CitrixConfig.txt"
        }
        
        if (Test-Path $ConfigFilePath) {
            Write-Host "Configuration loaded from: $ConfigFilePath" -ForegroundColor Cyan
...
if ($RelocateCDDVDDrive) {
    ...
    try {
        ...
        if (Test-Path "D:\") {
            ...
            # Detect if D: is a CD/DVD drive using multiple reliable methods
            $IsCDDVD = $false
            
            try {
                Write-Host "AGGRESSIVE CD/DVD DETECTION - Testing ALL methods..." -ForegroundColor Magenta
                
                # Method 1: Direct Win32_CDROMDrive WMI query (most specific)
                Write-Host "Method 1: Checking Win32_CDROMDrive..." -ForegroundColor Cyan
                try {
                    $CDROMDrives = Get-WmiObject -Class Win32_CDROMDrive -ErrorAction SilentlyContinue
                    if ($CDROMDrives) {
                        foreach ($Drive in $CDROMDrives) {
                            Write-Host "Found CD/DVD Drive: $($Drive.Drive) - $($Drive.Caption)" -ForegroundColor Yellow
                            if ($Drive.Drive -eq "D:") {
                                $IsCDDVD = $true
                                Write-Host "MATCH: D: drive IS a CD/DVD ROM (Win32_CDROMDrive)" -ForegroundColor Green
                                break
                            }
                        }
                    }
                } catch {
                    Write-Host "Win32_CDROMDrive query failed: $($_.Exception.Message)" -ForegroundColor Red
                }
                
                # Method 2: Check LogicalDisk DriveType=5
                if (-not $IsCDDVD) {
                    Write-Host "Method 2: Checking Win32_LogicalDisk DriveType..." -ForegroundColor Cyan
                    try {
                        $LogicalDisk = Get-WmiObject -Class Win32_LogicalDisk -Filter "DeviceID='D:'" -ErrorAction SilentlyContinue
                        if ($LogicalDisk) {
                            Write-Host "D: drive DriveType: $($LogicalDisk.DriveType)" -ForegroundColor White
                            if ($LogicalDisk.DriveType -eq 5) {
                                $IsCDDVD = $true
                                Write-Host "MATCH: D: drive IS a CD/DVD ROM (DriveType=5)" -ForegroundColor Green
                            }
                        }
                    } catch {
                ...
                # Method 3: Check with Get-Volume for CDFS/UDF
                if (-not $IsCDDVD) {
                    Write-Host "Method 3: Checking filesystem type..." -ForegroundColor Cyan
                    try {
                        $Volume = Get-Volume -DriveLetter D -ErrorAction SilentlyContinue
                        if ($Volume) {
                            Write-Host "D: FileSystem: $($Volume.FileSystem)" -ForegroundColor White
                            if ($Volume.FileSystem -eq "CDFS" -or $Volume.FileSystem -eq "UDF") {
                                $IsCDDVD = $true
                                Write-Host "MATCH: D: drive IS a CD/DVD ROM (FileSystem: $($Volume.FileSystem))" -ForegroundColor Green
                            }
                        }
                    } catch {
                        Write-Host "Volume query failed: $($_.Exception.Message)" -ForegroundColor Red
                    }
                }
                
                # Method 4: Write test (read-only check)
                if (-not $IsCDDVD) {
                    Write-Host "Method 4: Testing write capability..." -ForegroundColor Cyan
                    try {
                        $TestFile = "D:\cd_test_$(Get-Random).tmp"
                        "test" | Out-File -FilePath $TestFile -ErrorAction Stop
                        Remove-Item $TestFile -Force -ErrorAction SilentlyContinue
                        Write-Host "D: drive is writable - not CD/DVD" -ForegroundColor White
                    } catch {
                        $IsCDDVD = $true
                        Write-Host "MATCH: D: drive IS a CD/DVD ROM (read-only)" -ForegroundColor Green
                    }
                }
                
                # Method 5: Check diskpart list for CD/DVD
                if (-not $IsCDDVD) {
                    Write-Host "Method 5: Using diskpart to identify CD/DVD..." -ForegroundColor Cyan
                    try {
                        $DiskpartOutput = & diskpart /s (echo "list volume" | Out-File -FilePath "temp_list.txt" -Encoding ASCII; "temp_list.txt") 2>&1
                        Remove-Item "temp_list.txt" -Force -ErrorAction SilentlyContinue
                        
                        $DVDLine = $DiskpartOutput | Where-Object { $_ -match "D\s+.*DVD" -or $_ -match "D\s+.*CD" }
                        if ($DVDLine) {
                            $IsCDDVD = $true
                            Write-Host "MATCH: D: drive IS a CD/DVD ROM (diskpart: $DVDLine)" -ForegroundColor Green
                        }
                    } catch {
                        Write-Host "Diskpart query failed: $($_.Exception.Message)" -ForegroundColor Red
                    }
                }
                ...
                if ($IsCDDVD) {
                    Write-Host "FINAL RESULT: D: drive CONFIRMED as CD/DVD ROM" -ForegroundColor Green -BackgroundColor Black
                } else {
                    Write-Host "FINAL RESULT: D: drive is NOT a CD/DVD ROM" -ForegroundColor Red -BackgroundColor Black
                }
                Write-Host "" -ForegroundColor White
                
            } catch {
                Write-Host "CRITICAL ERROR in CD/DVD detection: $($_.Exception.Message)" -ForegroundColor Red
                Write-Host "Assuming D: is a regular drive" -ForegroundColor Yellow
            }
            
            if ($IsCDDVD) {
                Write-Host "D: drive is CD/DVD ROM - relocating to ${CDDVMTargetDrive}:..." -ForegroundColor Yellow
                
                # Check if target drive is available using simple path test
...
if ($RequireCacheDrive -and $ConfigureCacheDrive -and $UseVirtualCacheDrive) {
    ...
    try {
        ...
        $VirtualCacheResult = New-VirtualCacheDrive -DriveLetter $CacheDriveLetter -SizeMB $VirtualCacheDriveSizeMB -VHDXPath $VirtualCacheDrivePath -ConfigFilePath $ConfigFilePath
