# After log initialization, continue setup

# Set default export flags if none specified
$anyExportFlagSet = $ExportFarms -or $ExportSites -or $ExportStores -or $ExportCollections -or $ExportDevices -or $ExportvDisks -or $ExportViews
if (-not $anyExportFlagSet) {
    $ExportFarms = $true
    $ExportSites = $true
    $ExportStores = $true
    ...
    $ExportvDisks = $true
    $ExportViews = $true
    Write-Log -Message "No export components specified. Exporting all components." -Level "INFO"
}

Write-Log -Message "Starting Citrix PVS Configuration Export Script" -Level "INFO"
Write-Log -Message "Output folder: $OutputFolder" -Level "INFO"
Write-Log -Message "Log file: $LogFile" -Level "INFO"
Write-Log -Message "PVS Server: $PVSServer" -Level "INFO"

# Initialize PVS environment
if (!(Import-PVSSnapin)) {
    Write-Log -Message "Failed to initialize PVS environment. Exiting script." -Level "ERROR"
    exit 1
}

# Create output folder
if (!(New-OutputFolder)) {
    Write-Log -Message "Failed to create output folder. Exiting script." -Level "ERROR"
    exit 1
...
# Script Parameters
param (
    [string]$OutputFolder = "D:\PVSExport",
    [string]$LogFile = "D:\PVSExport\export.log",
    [string]$PVSServer = "",
    [switch]$ExportFarms,
    [switch]$ExportSites,
    [switch]$ExportStores,
    [switch]$ExportCollections,
    [switch]$ExportDevices,
    [switch]$ExportvDisks,
    [switch]$ExportViews
)
...
function Import-PVSSnapin {
    try {
        ...
        if (!(Get-PSSnapin -Name Citrix.PVS.SnapIn -ErrorAction SilentlyContinue)) {
            ...
            Write-Log -Message "Citrix PVS PowerShell snap-in loaded successfully" -Level "SUCCESS"
        } else {
            Write-Log -Message "Citrix PVS PowerShell snap-in is already loaded" -Level "INFO"
        }
        return $true
    }
    catch {
        Write-Log -Message "Failed to load Citrix PVS PowerShell snap-in: $_" -Level "ERROR"
        return $false
    }
}
...
function New-OutputFolder {
    try {
        ...
        if (!(Test-Path -Path $OutputFolder)) {
            ...
            Write-Log -Message "Created output folder: $OutputFolder" -Level "SUCCESS"
        } else {
            Write-Log -Message "Output folder already exists: $OutputFolder" -Level "INFO"
        }
