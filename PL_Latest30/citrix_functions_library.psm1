...
function Set-PageFile {
    ...
    try {
        ...
        $ComputerSystem = Get-WmiOrCimInstance -ClassName Win32_ComputerSystem
        if ($ComputerSystem.GetType().Name -eq "ManagementObject") {
            # WMI object - use alternative method to avoid Put() compatibility issues
            try {
                $ComputerSystem.AutomaticManagedPagefile = $false
                $ComputerSystem.Put() | Out-Null
            } catch {
                # Fallback: Use registry method for pagefile management
                Write-Log "WMI Put() method failed, using registry approach" "WARNING"
                Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" -Name "PagingFiles" -Value ""
            }
        } else {
            # CIM object - use Set-CimInstance
            Set-CimInstance -InputObject $ComputerSystem -Property @{AutomaticManagedPagefile = $false}
        }
        
        # Remove existing pagefiles
        $PageFiles = Get-WmiOrCimInstance -ClassName Win32_PageFileSetting
        foreach ($PageFile in $PageFiles) {
            if ($PageFile.GetType().Name -eq "ManagementObject") {
                # WMI object - use Delete method
                $PageFile.Delete()
            } else {
                # CIM object - use Remove-CimInstance
        ...
        try {
            ...
            # Fallback to WMI method for older PowerShell versions
            try {
                ...
                Write-Log "Both CIM and WMI pagefile creation methods failed: $($_.Exception.Message)" "ERROR"
                throw "Unable to create pagefile using available methods"
            }
        }
...
function Set-PowerPlan {
    ...
    try {
        ...
        if ($PowerPlan) {
            if ($PowerPlan.GetType().Name -eq "ManagementObject") {
                # WMI object - use Activate method
                $PowerPlan.Activate()
            } else {
                # CIM object - use Invoke-CimMethod
...
function New-VirtualCacheDrive {
    ...
    try {
        ...
        try {
            $VHD = New-VHD -Path $CacheVHDXPath -SizeBytes ($CacheDriveSizeMB * 1MB) -Dynamic
        ...
        # Mount VHDX
        try {
            $MountedVHD = Mount-VHD -Path $CacheVHDXPath -PassThru
            Write-Log "VHDX mounted successfully" "SUCCESS"
        } catch {
            $Results.Errors += "Failed to mount VHDX: $($_.Exception.Message)"
            Write-Log "Failed to mount VHDX: $($_.Exception.Message)" "ERROR"
            return $Results
        }
        
        # Initialize and format the disk
        try {
            $Disk = Get-Disk | Where-Object { $_.Location -eq $CacheVHDXPath }
            if ($Disk) {
                ...
                Write-Log "Virtual cache drive formatted successfully as ${DriveLetter}: ($VolumeLabel)" "SUCCESS"
                ...
                $Results.Errors += "Could not locate mounted disk"
                Write-Log "Could not locate mounted disk for VHDX" "ERROR"
            }
        } catch {
            $Results.Errors += "Failed to initialize/format virtual disk: $($_.Exception.Message)"
            Write-Log "Failed to initialize/format virtual disk: $($_.Exception.Message)" "ERROR"
        }
                ...
                if ($GhostDevices.Count -gt 0) {
                    ...
                    foreach ($Device in $GhostDevices) {
                        try {
                            ...
                            # Remove the device with CIM/WMI compatibility
                            if ($Device.GetType().Name -eq "ManagementObject") {
                                # WMI object - use Delete method
                                $Device.Delete()
                            } else {
                                # CIM object - use Remove-CimInstance
                                Remove-CimInstance -InputObject $Device
                            }
                            $RemovalCount++
