...
if ($RelocateCDDVDDrive) {
    ...
    try {
        ...
        if (Test-Path "D:\") {
            ...
            try {
                Write-Host "Checking for CD/DVD ROM on D: drive..." -ForegroundColor Cyan
                
                # Check if D: drive exists first
                if (-not (Test-Path "D:\")) {
                    Write-Host "D: drive not accessible - skipping CD/DVD detection" -ForegroundColor Red
                } else {
                    Write-Host "D: drive is accessible - proceeding with detection" -ForegroundColor Green
                    
                    # Primary method: Check all CD/DVD drives
                    ...
                    if ($CDROMDrives) {
                        ...
                        foreach ($Drive in $CDROMDrives) {
                            Write-Host "  CD/DVD Drive: $($Drive.Drive) - $($Drive.Caption)" -ForegroundColor Yellow
                            if ($Drive.Drive -eq "D:") {
                                $IsCDDVD = $true
                    ...
                    # Fallback: Check drive type
                    if (-not $IsCDDVD) {
                        Write-Host "Method 2: Checking LogicalDisk DriveType..." -ForegroundColor Cyan
                        ...
                        if ($LogicalDisk) {
                            ...
                            if ($LogicalDisk.DriveType -eq 5) {
                                $IsCDDVD = $true
                                Write-Host "  MATCH: D: drive is CD/DVD (DriveType=5)!" -ForegroundColor Green
                            }
                        } else {
                            Write-Host "  D: drive not found in LogicalDisk query" -ForegroundColor Yellow
                        }
                    }
                    
                    # Final fallback: Write test
                    if (-not $IsCDDVD) {
                        Write-Host "Method 3: Testing write capability..." -ForegroundColor Cyan
                        try {
                            $TestFile = "D:\cd_test_$((Get-Date).Ticks).tmp"
                            "test" | Out-File -FilePath $TestFile -ErrorAction Stop
                            Remove-Item $TestFile -Force -ErrorAction SilentlyContinue
                            Write-Host "  D: drive is writable - not CD/DVD" -ForegroundColor White
                        } catch {
                            $IsCDDVD = $true
                            Write-Host "  MATCH: D: drive is read-only - CD/DVD detected!" -ForegroundColor Green
                        }
                    }
                    
                    # Manual override prompt if still not detected
                    if (-not $IsCDDVD) {
                        Write-Host "" -ForegroundColor White
                        Write-Host "MANUAL OVERRIDE: Is D: drive a CD/DVD ROM? (y/n)" -ForegroundColor Yellow -NoNewline
                        $UserInput = Read-Host " "
                        if ($UserInput -eq "y" -or $UserInput -eq "Y" -or $UserInput -eq "yes") {
                            $IsCDDVD = $true
                            Write-Host "User confirmed: D: drive is CD/DVD ROM" -ForegroundColor Green
                        }
                    }
                }
                
            } catch {
                Write-Host "CD/DVD detection failed: $($_.Exception.Message)" -ForegroundColor Red
            }
