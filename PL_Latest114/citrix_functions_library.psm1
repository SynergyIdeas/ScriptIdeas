...
function Invoke-SystemOptimizations {
    ...
    try {
        ...
        return @{ OverallStatus = $true; Message = "System optimizations applied" }
    }
    catch {
...
function Invoke-VDIOptimizations {
    ...
    try {
        ...
        $VDIKeys = @(
            ...
            @{ Path = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer"; Name = "HideClock"; Value = 1 },
        ...
        return @{ OverallStatus = $true; Success = $true; Message = "VDI optimizations applied" }
    }
    catch {
...
function Invoke-ProfileCleanup {
    ...
    try {
        ...
        return @{ Success = $ProfileResult; Message = "Profile cleanup completed" }
    }
    catch {
...
function Optimize-NetworkSettings {
    ...
    try {
        ...
        return @{ Success = $true; Message = "Network optimizations applied" }
    }
    catch {
...
function Optimize-StorageSettings {
    ...
    try {
        ...
        return @{ Success = $true; Message = "Storage optimizations applied" }
    }
    catch {
...
function Remove-ActiveComponentsRegistry {
    ...
    try {
        ...
        if ($Global:CachedConfig -and $Global:CachedConfig.ActiveSetupComponentsToRemove) {
...
function Add-UberAgent {
    ...
    try {
        ...
        if ($InstallProcess.ExitCode -eq 0) {
            ...
            # Use environment-based config selection instead of default config path
            Write-Log "Starting environment-based UberAgent configuration selection..." "INFO"
            $EnvironmentConfigResult = Copy-UberAgentConfigByEnvironment -ConfigFilePath $ConfigFilePath
            
            if ($EnvironmentConfigResult.Success) {
                Write-Log "Environment-based config copied successfully" "SUCCESS"
                Write-Log "Environment: $($EnvironmentConfigResult.Environment)" "INFO"
                Write-Log "Source: $($EnvironmentConfigResult.SourcePath)" "INFO"
                Write-Log "Destination: $($EnvironmentConfigResult.DestinationPath)" "INFO"
                $Results.ConfigCopied = $true
                $Results.FilesProcessed += "Config: uberagent.conf [ENV-$($EnvironmentConfigResult.Environment)]"
            } else {
                $ErrorMsg = "Environment-based config copy failed: $($EnvironmentConfigResult.Error)"
                Write-Log $ErrorMsg "ERROR"
                $Results.Errors += $ErrorMsg
                $Results.ConfigCopied = $false
            }
...
Export-ModuleMember -Function @(
   ...
   'Remove-PasswordAgeRegistryKey', 'Remove-WEMRSAKey', 'New-InstallConfig',
   ...
   'Get-MachineEnvironment', 'Test-IPInRange', 'Copy-UberAgentConfigByEnvironment'
)
...
function Get-MachineEnvironment {
    ...
    try {
        Write-Log "Detecting machine environment based on IP address..." "INFO"
        ...
        if (-not $NetworkAdapters) {
            Write-Log "No valid IP addresses found for environment detection" "ERROR"
            return @{
                Success = $false
                Environment = "UNKNOWN"
                IPAddress = $null
        ...
        # Read IP ranges from config file
        $DevIPRanges = Get-ConfigValue -Key "DevIPRanges" -DefaultValue "" -ConfigFile $ConfigFilePath
        $ProdIPRanges = Get-ConfigValue -Key "ProdIPRanges" -DefaultValue "" -ConfigFile $ConfigFilePath
        
        if ([string]::IsNullOrEmpty($DevIPRanges) -or [string]::IsNullOrEmpty($ProdIPRanges)) {
            Write-Log "IP ranges not configured in config file" "ERROR"
            return @{
                Success = $false
                Environment = "UNKNOWN"
                IPAddress = $null
                Error = "IP ranges not configured"
            }
        }
        
        # Parse IP ranges
        $DevRanges = $DevIPRanges -split ',' | ForEach-Object { $_.Trim() }
        $ProdRanges = $ProdIPRanges -split ',' | ForEach-Object { $_.Trim() }
        
        Write-Log "Dev IP Ranges: $($DevRanges -join ', ')" "INFO"
        Write-Log "Prod IP Ranges: $($ProdRanges -join ', ')" "INFO"
        
        # Check each network adapter IP against ranges
        foreach ($Adapter in $NetworkAdapters) {
            $CurrentIP = $Adapter.IPAddress
            Write-Log "Checking IP address: $CurrentIP" "INFO"
            
            # Check against DEV ranges
            foreach ($Range in $DevRanges) {
                if (Test-IPInRange -IPAddress $CurrentIP -CIDRRange $Range) {
                    Write-Log "IP $CurrentIP matches DEV range $Range" "SUCCESS"
                    return @{
                        Success = $true
                        Environment = "DEV"
                        IPAddress = $CurrentIP
            ...
            # Check against PROD ranges
            foreach ($Range in $ProdRanges) {
                if (Test-IPInRange -IPAddress $CurrentIP -CIDRRange $Range) {
                    Write-Log "IP $CurrentIP matches PROD range $Range" "SUCCESS"
                    return @{
                        Success = $true
                        Environment = "PROD"
                        IPAddress = $CurrentIP
        ...
        # No matches found
        Write-Log "No IP addresses match configured ranges. Available IPs: $($NetworkAdapters.IPAddress -join ', ')" "WARN"
        return @{
            Success = $false
            Environment = "UNKNOWN"
            IPAddress = $NetworkAdapters[0].IPAddress
            Error = "IP address does not match any configured environment ranges"
        }
    }
    catch {
        Write-Log "Failed to detect machine environment: $($_.Exception.Message)" "ERROR"
        return @{
            Success = $false
            Environment = "UNKNOWN"
            IPAddress = $null
...
function Test-IPInRange {
    ...
    try {
        ...
        if ($CIDRRange -match '^(\d+\.\d+\.\d+\.\d+)/(\d+)$') {
            $NetworkIP = $matches[1]
            $SubnetBits = [int]$matches[2]
        } else {
            Write-Log "Invalid CIDR format: $CIDRRange" "ERROR"
            return $false
        }
        ...
        $MaskBytes = @(0, 0, 0, 0)
        $BitsToSet = $SubnetBits
        for ($i = 0; $i -lt 4; $i++) {
            if ($BitsToSet -ge 8) {
                $MaskBytes[$i] = 255
                $BitsToSet -= 8
            } elseif ($BitsToSet -gt 0) {
                $MaskBytes[$i] = (255 -shl (8 - $BitsToSet)) -band 255
                $BitsToSet = 0
            }
        }
        ...
        Write-Log "Error testing IP range: $($_.Exception.Message)" "ERROR"
        return $false
    }
}

function Copy-UberAgentConfigByEnvironment {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$true)]
        [string]$ConfigFilePath
    ...
    try {
        Write-Log "Starting environment-based UberAgent config selection..." "INFO"
        
        # Detect machine environment
        $EnvironmentResult = Get-MachineEnvironment -ConfigFilePath $ConfigFilePath
        
        if (-not $EnvironmentResult.Success) {
            Write-Log "Environment detection failed: $($EnvironmentResult.Error)" "ERROR"
            return @{
                Success = $false
                Environment = "UNKNOWN"
                ConfigCopied = $false
        ...
        Write-Log "Detected environment: $Environment (IP: $($EnvironmentResult.IPAddress))" "SUCCESS"
        
        # Get appropriate config source path
        $ConfigSourcePath = ""
        if ($Environment -eq "DEV") {
            $ConfigSourcePath = Get-ConfigValue -Key "UberAgentDevConfigSource" -DefaultValue "" -ConfigFile $ConfigFilePath
        } elseif ($Environment -eq "PROD") {
            $ConfigSourcePath = Get-ConfigValue -Key "UberAgentProdConfigSource" -DefaultValue "" -ConfigFile $ConfigFilePath
        } else {
        ...
        if ([string]::IsNullOrEmpty($ConfigSourcePath)) {
            Write-Log "Config source path not configured for $Environment environment" "ERROR"
            return @{
                Success = $false
                Environment = $Environment
                ConfigCopied = $false
                Error = "Config source path not configured"
            }
        }
        
        # Expand environment variables in path
        $ConfigSourcePath = Expand-ConfigPath -Path $ConfigSourcePath
        Write-Log "Using $Environment config source: $ConfigSourcePath" "INFO"
        
        # Get destination path from config file
        $ConfigDestPath = Get-ConfigValue -Key "UberAgentConfigLocalPath" -DefaultValue "C:\Program Files\vast limits\uberAgent\config\uberagent.conf" -ConfigFile $ConfigFilePath
        Write-Log "Config destination: $ConfigDestPath" "INFO"
        
        # Ensure destination directory exists
        $DestDir = Split-Path $ConfigDestPath -Parent
        if (-not (Test-Path $DestDir)) {
            New-Item -Path $DestDir -ItemType Directory -Force | Out-Null
        ...
        if (Test-Path $ConfigSourcePath) {
            ...
            Write-Log "Copied $Environment UberAgent config to: $ConfigDestPath" "SUCCESS"
            ...
            return @{
                ...
                DestinationPath = $ConfigDestPath
                IPAddress = $EnvironmentResult.IPAddress
                MatchedRange = $EnvironmentResult.MatchedRange
            }
        } else {
            Write-Log "Source config file not found: $ConfigSourcePath" "ERROR"
            return @{
                Success = $false
                Environment = $Environment
                ConfigCopied = $false
                Error = "Source config file not found: $ConfigSourcePath"
            }
        }
    }
    catch {
        Write-Log "Failed to copy UberAgent config by environment: $($_.Exception.Message)" "ERROR"
        return @{
            Success = $false
            Environment = "UNKNOWN"
            ConfigCopied = $false
...
Export-ModuleMember -Function @(
   'Write-Log', 'Get-ConfigValue', 'Expand-ConfigPath', 'Get-OSVersion',
   'Install-VDAFromISO', 'Install-PVSFromISO', 'Add-WEMAgent', 'Add-UberAgent', 'Set-IBMTADDMPermissions', 'Set-IBMTADDMSCMPermissions',
   'Get-CacheDrive', 'New-CacheDrive', 'Set-PageFile', 'Clear-EventLogs', 'Disable-WindowsUpdates',
   'Set-TimeZone', 'Join-Domain', 'Set-LocalUserPassword',
   'Install-CitrixVDA', 'Install-CitrixPVSTarget', 'Install-CitrixWEMAgent', 'Install-UberAgent',
