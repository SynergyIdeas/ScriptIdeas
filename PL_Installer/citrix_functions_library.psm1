...
function Install-CitrixVDA {
    ...
    try {
        ...
        Write-Log "ISO path: $VDAISOPath"
        
        if (!(Test-Path $VDAISOPath)) {
            throw "VDA ISO not found at: $VDAISOPath"
        }
...
function Install-PVSTargetDevice {
    ...
    try {
        if ([string]::IsNullOrEmpty($PVSISOPath) -or !(Test-Path $PVSISOPath)) {
            Write-Log "PVS Target Device installation skipped - ISO not found"
            $Results.Skipped = $true
            return $Results
        }
        
        Write-Log "Installing PVS Target Device from ISO..."
        Write-Log "ISO path: $PVSISOPath"
