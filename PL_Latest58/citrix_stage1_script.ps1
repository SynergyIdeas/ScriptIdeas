            ...
            if ($NetworkAdapters) {
                foreach ($Adapter in $NetworkAdapters) {
                    ...
                    foreach ($DNSServer in $ServerAddresses) {
                        try {
                            $DNSTest = Test-NetConnection -ComputerName $DNSServer -Port 53 -WarningAction SilentlyContinue -ErrorAction Stop -InformationLevel Quiet
            ...
            if ($DomainResolution) {
                ...
                try {
                    ...
                    if ($DCRecords) {
                        ...
                        # Test LDAP port (389)
                        try {
                            $LDAPTest = Test-NetConnection -ComputerName $FirstDC -Port 389 -WarningAction SilentlyContinue -ErrorAction Stop -InformationLevel Quiet
                        ...
                        try {
                            $KerberosTest = Test-NetConnection -ComputerName $FirstDC -Port 88 -WarningAction SilentlyContinue -ErrorAction Stop -InformationLevel Quiet
                        ...
                        if ($JoinDomain) {
                            ...
                            foreach ($PortTest in $DomainJoinPorts) {
                                try {
                                    $ConnTest = Test-NetConnection -ComputerName $FirstDC -Port $PortTest.Port -WarningAction SilentlyContinue -ErrorAction Stop -InformationLevel Quiet
...
"@
   ...
   try {
       ...
       $ReportPath = New-CitrixReport -Stage 1 -InstallResults $InstallConfig.InstallationResults -ValidationResults $FinalValidation -OpenInBrowser $true
   ...
   try {
       if ($InstallConfig) {
           ...
           Save-InstallationConfig -Config $InstallConfig -ConfigPath "$LocalInstallPath\CitrixConfig.json"
       }
   }
   catch {
