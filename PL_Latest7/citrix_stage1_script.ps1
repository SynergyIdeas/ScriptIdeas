...
if ($DDriveCheck -and $DDriveCheck.DriveType -eq 5) {
    Write-Host "D: drive is CD/DVD ROM - relocating to Z: drive..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($CDVolume) {
            ...
            if (-not $ZDriveExists) {
                Write-Host "Moving CD/DVD ROM from D: to Z:..." -ForegroundColor Cyan
                $CDVolume | Set-CimInstance -Property @{ DriveLetter = "Z:" }
                Start-Sleep -Seconds 3
                
                # Verify relocation
                $DDriveAfter = Get-CimInstance -ClassName Win32_LogicalDisk | Where-Object { $_.DeviceID -eq "D:" }
                if (-not $DDriveAfter) {
                    Write-Host "CD/DVD ROM successfully moved to Z: drive" -ForegroundColor Green
                ...
                Write-Host "ERROR: Z: drive already exists - cannot relocate CD/DVD ROM" -ForegroundColor Red
        ...
        Write-Host "Please manually change CD/DVD ROM drive letter to Z: and restart" -ForegroundColor Yellow
        Write-Host "INSTALLATION TERMINATED" -ForegroundColor Red
        exit 1
    }
    ...
    $DCacheAttached = Request-DCacheDrive
    
    # Force immediate termination if Request-DCacheDrive returns false
...
try {
    ...
    $CitrixOptimizerResult = Invoke-CitrixOptimizer -ConfigFilePath $ConfigFilePath
    ...
    $CitrixServicesResult = Disable-CitrixServices -ConfigFilePath $ConfigFilePath
    ...
    if (![string]::IsNullOrEmpty($UberAgentPath)) {
        ...
        $UberAgentResult = Install-UberAgent -UberAgentPath $UberAgentPath -ConfigFilePath $ConfigFilePath
    ...
    if ($JoinDomain) {
        ...
        if (![string]::IsNullOrEmpty($DomainName)) {
            ...
            if ($DomainCredential) {
                ...
                $DomainJoinResult = Join-Domain @DomainJoinParams
