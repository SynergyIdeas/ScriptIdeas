...
function New-CitrixReport {
    ...
    # Write HTML file
    try {
        # Use the configured OutputPath that was properly constructed earlier
        $ReportPath = $OutputPath
        
        # Ensure the output directory exists
        $OutputDirectory = Split-Path $ReportPath -Parent
        ...
        if ($OpenInBrowser) {
            try {
                # Get the actual file system path without PowerShell provider prefix
                ...
                # Handle network paths vs local paths differently
                if ($FullPath.StartsWith("\\")) {
                    # Network path: \\server\share\file.html -> file://server/share/file.html
                    $NetworkPath = $FullPath.Substring(2) -replace '\\', '/'
                    $FileUrl = "file://$NetworkPath"
                } else {
                    # Local path: C:\folder\file.html -> file:///C:/folder/file.html
