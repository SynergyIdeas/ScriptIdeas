...
try {
    ...
    try {
        $NTPConfigResult = Configure-NTPTimeSources -ConfigFilePath $ConfigFilePath
        $NTPResult = $NTPConfigResult  # Keep backward compatibility
    ...
    if ($RequireCacheDrive) {
        ...
        if ($CachedConfig.ConfigurePagefile -and $CachedConfig.RedirectPagefileToCache) {
            ...
            $PagefileResult = Set-PageFile -SizeGB $CachedConfig.PagefileSizeGB -DriveLetter $CachedConfig.CacheDriveLetter
        } elseif ($CachedConfig.ConfigurePagefile) {
            Write-Log "Configuring pagefile without cache redirection..."
            $PagefileResult = Set-PageFile -SizeGB $CachedConfig.PagefileSizeGB -DriveLetter "C"
        } else {
    ...
    Write-LogHeader "UBERAGENT CONFIG MANAGEMENT"
    Write-Log "Managing UberAgent configuration files..." "INFO"
    
    try {
        # Check if function is available, if not, try to reload module then skip gracefully
        if (Get-Command Copy-UberAgentConfigs -ErrorAction SilentlyContinue) {
            $UberAgentConfigResult = Copy-UberAgentConfigs -ConfigFilePath $ConfigFilePath
        } else {
            Write-Log "Copy-UberAgentConfigs function not available, attempting module reload..." "WARN"
            
            # Try to reload the module to get the latest functions
            try {
                $FunctionsPath = Join-Path $PSScriptRoot "citrix_functions_library.psm1"
                if (Test-Path $FunctionsPath) {
                    Get-Module citrix_functions_library | Remove-Module -Force -ErrorAction SilentlyContinue
                    Import-Module $FunctionsPath -Force -ErrorAction Stop -DisableNameChecking -Global
                    Write-Log "Module reloaded successfully" "INFO"
                    
                    # Try again after reload
                    if (Get-Command Copy-UberAgentConfigs -ErrorAction SilentlyContinue) {
                        $UberAgentConfigResult = Copy-UberAgentConfigs -ConfigFilePath $ConfigFilePath
                        Write-Log "Function available after module reload" "SUCCESS"
                    } else {
                        throw "Function still not available after reload"
                    }
                } else {
                    throw "Functions module file not found"
                }
            } catch {
                Write-Log "Module reload failed: $($_.Exception.Message)" "ERROR"
                $UberAgentConfigResult = @{
                    Success = $true
                    Skipped = $true
                    ConfigsCopied = 0
                    Message = "UberAgent config management skipped - function not available"
                    Details = @("Copy-UberAgentConfigs function not found in module", "Module reload attempted but failed")
                }
            }
        }
        
        if ($UberAgentConfigResult.Success) {
            Write-Log "UberAgent config management completed successfully" "SUCCESS"
            Write-Log "Configs copied: $($UberAgentConfigResult.ConfigsCopied)" "SUCCESS"
            Write-Log "Dev config: $($UberAgentConfigResult.DevConfigPath)" "INFO"
            Write-Log "Prod config: $($UberAgentConfigResult.ProdConfigPath)" "INFO"
        } else {
            Write-Log "UberAgent config management completed with issues" "WARN"
            if ($UberAgentConfigResult.Error) {
                Write-Log "Error: $($UberAgentConfigResult.Error)" "ERROR"
            }
        }
    }
    catch {
        Write-Log "Failed to manage UberAgent configs: $($_.Exception.Message)" "ERROR"
        $UberAgentConfigResult = @{
            Success = $false
            Error = $_.Exception.Message
            Message = "UberAgent config management failed"
        }
    }
    
    # Check Installed Software Versions
    Write-LogHeader "INSTALLED SOFTWARE VERSIONS"
    Write-Log "Checking versions of installed software..." "INFO"
    
    try {
        # Check if function is available, if not, try to reload module then skip gracefully
        if (Get-Command Get-InstalledSoftwareVersions -ErrorAction SilentlyContinue) {
            $SoftwareVersionsResult = Get-InstalledSoftwareVersions
        } else {
            Write-Log "Get-InstalledSoftwareVersions function not available, attempting module reload..." "WARN"
            ...
            try {
                ...
                Write-Log "Module reload failed: $($_.Exception.Message)" "ERROR"
                $SoftwareVersionsResult = @{
                    Success = $false
                    Skipped = $true
                    Message = "Software version check skipped - function not available"
                    Details = @("Get-InstalledSoftwareVersions function not found in module", "Module reload attempted but failed")
                }
            }
        }
        
        if ($SoftwareVersionsResult.Success) {
            Write-Log "Software version check completed successfully" "SUCCESS"
            if ($SoftwareVersionsResult.VDAVersion) {
                Write-Log "Citrix VDA: $($SoftwareVersionsResult.VDAVersion)" "SUCCESS"
            }
            if ($SoftwareVersionsResult.PVSVersion) {
                Write-Log "PVS Target Device: $($SoftwareVersionsResult.PVSVersion)" "SUCCESS"
            }
            if ($SoftwareVersionsResult.WEMVersion) {
                Write-Log "WEM Agent: $($SoftwareVersionsResult.WEMVersion)" "SUCCESS"
            }
            if ($SoftwareVersionsResult.TADDMVersion) {
                Write-Log "IBM TADDM: $($SoftwareVersionsResult.TADDMVersion)" "SUCCESS"
            }
            if ($SoftwareVersionsResult.UberAgentVersion) {
                Write-Log "UberAgent: $($SoftwareVersionsResult.UberAgentVersion)" "SUCCESS"
            }
        } else {
            Write-Log "Software version check completed with issues" "WARN"
            if ($SoftwareVersionsResult.Error) {
                Write-Log "Error: $($SoftwareVersionsResult.Error)" "ERROR"
            }
        }
    }
    catch {
        Write-Log "Failed to check software versions: $($_.Exception.Message)" "ERROR"
        $SoftwareVersionsResult = @{
            Success = $false
            Error = $_.Exception.Message
            Message = "Software version check failed"
        }
    }
    ...
    $AllValidationResults = @{
        ...
        "UberAgent Config Management" = if ($UberAgentConfigResult) { $UberAgentConfigResult } else { @{ Success = $false; Message = "UberAgent config management not executed"; Skipped = $true } }
        "Virtual Cache Drive Removal" = @{ Success = $false; Skipped = $true; Message = "Will be updated dynamically later" }
        "Installed Software Versions" = if ($SoftwareVersionsResult) { $SoftwareVersionsResult } else { @{ Success = $false; Message = "Software version check not executed"; Skipped = $true } }
    }
    ...
    if (-not $ReportGenerated) {
        ...
        foreach ($key in $Stage2Results.Keys) {
            ...
            if ($result -is [hashtable]) {
                ...
                if ($key -eq ".NET Framework Optimization" -and !$result.Success -and $result.Error -match "ngen|not found") {
                    $isSkipped = $true
                }
                
                if ($key -eq "System Defragmentation" -and !$result.Success -and $result.Error -match "disabled|skipped") {
                    $isSkipped = $true
                }
                
                if ($key -eq "UberAgent Service Configuration" -and !$result.Success -and $result.Error -match "not found|not configured") {
                    $isSkipped = $true
                }
                ...
                if ($key -eq "NTP Time Configuration" -and !$result.Success -and $result.Error -match "disabled|not configured") {
                    $isSkipped = $true
                }
