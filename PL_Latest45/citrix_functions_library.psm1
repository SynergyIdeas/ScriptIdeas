...
function Install-VDAFromISO {
    ...
    try {
        ...
        # Mount ISO to detect VDA installer automatically
        $MountResult = Mount-DiskImage -ImagePath $VDAISOPath -PassThru
        $DriveLetter = ($MountResult | Get-Volume).DriveLetter
        $MountPath = "${DriveLetter}:"
        
        # Auto-detect VDA installer
        $VDAInstallerPath = Find-VDAInstaller -MountPath $MountPath
        
        if (-not $VDAInstallerPath) {
        ...
        $InstallerName = Split-Path -Leaf $VDAInstallerPath
        Write-Log "Auto-detected VDA installer: $InstallerName at $VDAInstallerPath" "SUCCESS"
        
        # Dismount temporarily for Install-FromISO function to handle mounting
        Dismount-DiskImage -ImagePath $VDAISOPath -ErrorAction SilentlyContinue
        
        # Install VDA from ISO using auto-detected installer
        $InstallResult = Install-FromISO -ISOPath $VDAISOPath -InstallerName $InstallerName -InstallerArgs $VDAArgs -ProductName "Citrix VDA" -TimeoutMinutes 45
...
function Install-PVSFromISO {
    ...
    try {
        ...
        # Mount ISO to detect PVS installer automatically
        ...
        # Install PVS Target from ISO using auto-detected installer
        $InstallResult = Install-FromISO -ISOPath $PVSISOPath -InstallerName $InstallerName -InstallerArgs $PVSArgs -ProductName "PVS Target Device" -TimeoutMinutes 20
