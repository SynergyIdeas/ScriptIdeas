...
# Skip execution policy on non-Windows platforms
...
if ($RelocateCDDVDDrive) {
    ...
    # Always check for CD/DVD drives regardless of D: drive existence
    $IsCDDVD = $false
    
    Write-Host "Scanning system for all CD/DVD drives..." -ForegroundColor Green
    
    # Comprehensive CD/DVD detection using multiple methods
    Write-Host "Running comprehensive CD/DVD detection..." -ForegroundColor Cyan
    
    try {
        
        # Method 1: Win32_CDROMDrive WMI query
        try {
            ...
            if ($CDROMDrives) {
                ...
                foreach ($Drive in $CDROMDrives) {
                    Write-Host "  CD/DVD: $($Drive.Drive) - $($Drive.Caption)" -ForegroundColor Yellow
                    if ($Drive.Drive -eq "D:") {
                        $IsCDDVD = $true
                        Write-Host "D: drive is CD/DVD ROM - will relocate" -ForegroundColor Green
                        break
                    }
                }
            }
        } catch {
            Write-Host "WMI CD/DVD query failed: $($_.Exception.Message)" -ForegroundColor Red
        }
        
        # Method 2: LogicalDisk DriveType check
        if (-not $IsCDDVD) {
            try {
                $LogicalDisk = Get-WmiObject -Class Win32_LogicalDisk -Filter "DeviceID='D:'" -ErrorAction SilentlyContinue
                if ($LogicalDisk -and $LogicalDisk.DriveType -eq 5) {
                    $IsCDDVD = $true
                    Write-Host "D: drive detected as CD/DVD (DriveType=5)" -ForegroundColor Green
                }
            } catch {
                Write-Host "LogicalDisk query failed: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
        
        # Method 3: Volume filesystem check
        if (-not $IsCDDVD) {
            try {
                $Volume = Get-Volume -DriveLetter D -ErrorAction SilentlyContinue
                if ($Volume -and ($Volume.FileSystem -eq "CDFS" -or $Volume.FileSystem -eq "UDF")) {
                    $IsCDDVD = $true
                    Write-Host "D: drive has CD/DVD filesystem ($($Volume.FileSystem))" -ForegroundColor Green
                }
            } catch {
        ...
        if (-not $IsCDDVD) {
            try {
                $CDROMKey = Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\cdrom\Enum" -ErrorAction SilentlyContinue
                if ($CDROMKey) {
                    $CDROMCount = $CDROMKey.Count
                    if ($CDROMCount -gt 0) {
                        Write-Host "Registry indicates $CDROMCount CD/DVD drive(s) present" -ForegroundColor Cyan
                        # Assume D: is CD/DVD if registry shows CD/DVD drives exist
                        $IsCDDVD = $true
                        Write-Host "Assuming D: is CD/DVD based on registry detection" -ForegroundColor Green
                    }
                }
            } catch {
                Write-Host "Registry CD/DVD check failed: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
        ...
        if ($IsCDDVD) {
            ...
            if (-not (Test-Path "${CDDVMTargetDrive}:\")) {
                ...
                try {
                    # Use Windows diskpart for CD/DVD relocation
                    Write-Host "Using Windows diskpart to relocate CD/DVD ROM from D: to ${CDDVMTargetDrive}:..." -ForegroundColor Cyan
...
"@
                    ...
                    Write-Host "Windows CD/DVD relocation completed successfully" -ForegroundColor Green
                    Write-Host "D: drive letter is now available for Windows cache drive" -ForegroundColor Green
            ...
            Write-Host "No CD/DVD relocation needed - proceeding normally" -ForegroundColor Green
        }
...
$VirtualCacheDriveLabel = Get-ConfigValue -Key "VirtualCacheDriveLabel" -DefaultValue "Cache" -ConfigFile $ConfigFilePath
...
if ($RequireCacheDrive -and $ConfigureCacheDrive -and $UseVirtualCacheDrive) {
    ...
    try {
        ...
        $VirtualCacheResult = New-VirtualCacheDrive -DriveLetter $CacheDriveLetter -SizeMB $VirtualCacheDriveSizeMB -VHDXPath $VirtualCacheDrivePath -ConfigFilePath $Global:DefaultConfigPath
...
# Non-interactive mode - skip user prompts for automated testing
Write-Host "`nStarting installation in automated mode..." -ForegroundColor Yellow
...
try {
    ...
    # Configure Print Spooler service before VDA installation
    Write-LogHeader "Print Spooler Configuration"
    $ConfigureSpooler = [bool](Get-ConfigValue -Key "ConfigureSpooler" -DefaultValue "true" -ConfigFile $ConfigFilePath)
    if ($ConfigureSpooler) {
        Write-Log "Configuring Print Spooler service for VDA compatibility..."
        try {
            # Stop Print Spooler service
            Stop-Service -Name "Spooler" -Force -ErrorAction SilentlyContinue
            Write-Log "Print Spooler service stopped" "SUCCESS"
            
            # Set to Manual start
            Set-Service -Name "Spooler" -StartupType Manual
            Write-Log "Print Spooler startup type set to Manual" "SUCCESS"
            
            $InstallConfig.InstallationResults.SpoolerConfig = @{
                Success = $true
                Status = "Configured for VDA compatibility"
            }
        } catch {
            Write-Log "Print Spooler configuration failed: $($_.Exception.Message)" "WARN"
            $InstallConfig.InstallationResults.SpoolerConfig = @{
                Success = $false
        ...
        Write-Log "Print Spooler configuration skipped - disabled in configuration"
        $InstallConfig.InstallationResults.SpoolerConfig = @{ Skipped = $true }
    }
    
    # All other Windows system optimizations moved to Stage 2
    ...
    if ($JoinDomain) {
        ...
        $PrimaryDNSSuffix = Get-ConfigValue -Key "PrimaryDNSSuffix" -DefaultValue "domain.local" -ConfigFile $ConfigFilePath
        $DNSSuffixSearchList = Get-ConfigValue -Key "DNSSuffixSearchList" -DefaultValue "domain.local" -ConfigFile $ConfigFilePath
        $AppendPrimarySuffixes = [bool](Get-ConfigValue -Key "AppendPrimarySuffixes" -DefaultValue "true" -ConfigFile $ConfigFilePath)
        $AppendParentSuffixes = [bool](Get-ConfigValue -Key "AppendParentSuffixes" -DefaultValue "true" -ConfigFile $ConfigFilePath)
        $RegisterThisConnectionsAddress = [bool](Get-ConfigValue -Key "RegisterThisConnectionsAddress" -DefaultValue "true" -ConfigFile $ConfigFilePath)
...
"@
    ...
    Write-Host "Exiting due to fatal error..." -ForegroundColor Red
    Start-Sleep -Seconds 3
    exit 1
}
