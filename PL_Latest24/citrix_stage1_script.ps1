...
try {
    ...
    if (-not (Get-Command "Get-ConfigValue" -ErrorAction SilentlyContinue)) {
        ...
        function Get-CimInstance {
            param($ClassName)
            # Return empty object for Linux compatibility
            return @{ Caption = "Linux"; Version = "Unknown"; BuildNumber = "Unknown" }
        }
    }
...
if ($RelocateCDDVDDrive) {
    ...
    try {
        # Check if D: exists and is a CD/DVD ROM
        $DDriveCheck = $null
        try {
            $DDriveCheck = Get-CimInstance -ClassName Win32_LogicalDisk | Where-Object { $_.DeviceID -eq "D:" }
        } catch {
            Write-Host "Get-CimInstance not available - using fallback CD/DVD detection" -ForegroundColor Yellow
            # Fallback: assume D: needs checking if it exists
            if (Test-Path "D:\") {
                $DDriveCheck = @{ DeviceID = "D:"; DriveType = 5 }  # Assume CD/DVD
            }
        }
        ...
        if ($DDriveCheck -and $DDriveCheck.DriveType -eq 5) {
            ...
            try {
                ...
                # Fallback check
                $YDriveExists = if (Test-Path "Y:\") { @{ DeviceID = "Y:" } } else { $null }
            }
            ...
            if (-not $YDriveExists) {
                ...
                try {
                    ...
                    if ($CDVolume) {
                        ...
                        Write-Host "CD/DVD volume not found via WMI - trying diskpart method..." -ForegroundColor Yellow
                        
                        # Fallback to diskpart method
                        $DiskpartScript = @"
list volume
select volume D
assign letter=Y
...
"@
            ...
            Write-Host "No CD/DVD ROM detected on D: drive - proceeding normally" -ForegroundColor Green
        }
...
# Add fallback for Get-CimInstance in Linux environments
try {
    $DDriveCheck = Get-CimInstance -ClassName Win32_LogicalDisk | Where-Object { $_.DeviceID -eq "D:" }
} catch {
    Write-Host "Get-CimInstance not available - using fallback drive detection" -ForegroundColor Yellow
    $DDriveCheck = $null
    if (Test-Path "D:\") {
