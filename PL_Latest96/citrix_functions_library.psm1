...
function Set-DNSSuffix {
    ...
    try {
        ...
        # Read DNS settings from cached config or configuration file
        if ($Global:CachedConfig -and $Global:CachedConfig.PrimaryDNSSuffix) {
            $PrimaryDNSSuffix = $Global:CachedConfig.PrimaryDNSSuffix
            $DNSSuffixSearchListStr = $Global:CachedConfig.DNSSuffixSearchList -or ""
            $AppendPrimarySuffixes = [bool]($Global:CachedConfig.AppendPrimarySuffixes -or "true")
            $AppendParentSuffixes = [bool]($Global:CachedConfig.AppendParentSuffixes -or "true")
            $RegisterThisConnectionsAddress = [bool]($Global:CachedConfig.RegisterThisConnectionsAddress -or "true")
        } else {
                ...
                if ($IsDomainDir) {
                    try {
                        ...
                        try {
                            ...
                            robocopy $TempEmptyDir $Dir.FullName /MIR /R:0 /W:0 /NFL /NDL /NJH /NJS 2>&1 | Out-Null
...
function Remove-WEMRSAKey {
    ...
    try {
        ...
        if ($Results.RemovedKeys.Count -eq 0) {
            ...
            $Results.Success = $true  # Success if no keys exist
        } else {
...
function Start-CitrixOptimizer {
   ...
   try {
       ...
       # Always read configuration directly from config file for optimizer
       Write-Log "Reading Citrix Optimizer configuration directly from config file" "INFO"
       ...
       $TemplateNames = Get-ConfigValue -Key "CitrixOptimizerTemplate" -DefaultValue "Windows_Server_2019_VDI.xml" -ConfigFile $ConfigFilePath
       $OutputPath = Get-ConfigValue -Key "CitrixOptimizerOutputPath" -DefaultValue "C:\Temp\CitrixOptimizer_Results" -ConfigFile $ConfigFilePath
       ...
       if ([string]::IsNullOrEmpty($OptimizerPath) -or -not (Test-Path $OptimizerPath)) {
           Write-Log "Citrix Optimizer tool not found at path: $OptimizerPath" "ERROR"
           Write-Log "Please ensure Citrix Optimizer is installed and the path is correct in configuration" "ERROR"
           Write-Log "Original config path: %NetworkSourcePath%\\tools\\CitrixOptimizer\\CitrixOptimizer.exe" "INFO"
           Write-Log "NetworkSourcePath should resolve to: \\\\fileserver\\citrix" "INFO"
           $Results.Success = $false
           $Results.Error = "Citrix Optimizer executable not found at: $OptimizerPath"
           return $Results
       }
       ...
       if ($MissingTemplates.Count -gt 0) {
           ...
           if (Test-Path $TemplatesDirectory) {
               Get-ChildItem -Path $TemplatesDirectory -Filter "*.xml" | ForEach-Object {
...
function Clear-ActiveSetupComponents {
    [CmdletBinding()]
    param()
    
    try {
        Write-Log "Cleaning up Active Setup installed components..." "INFO"
        
        $ActiveSetupPath = "HKLM:\SOFTWARE\Microsoft\Active Setup\Installed Components"
        $RemovedCount = 0
        $PreservedCount = 0
        
        # Components to preserve (critical Windows and .NET components)
        $PreserveComponents = @(
            "{89820200-ECBD-11cf-8B85-00AA005B4340}",  # Windows Desktop Update
            "{89820200-ECBD-11cf-8B85-00AA005B4383}",  # IE6 SP1
            "{89B4C1CD-B018-4511-B0A1-5476DBF70820}",  # Microsoft .NET Framework
            "{9381D8F2-0288-11D0-9501-00AA00B911A5}",  # Microsoft FrontPage
            "{ACC563BC-4266-43f0-B6ED-9D38C4202C7E}",  # Microsoft .NET Framework
            ">{22d6f312-b0f6-11d0-94ab-0080c74c7e95}",  # Microsoft Windows Media Player
            ">{26923b43-4d38-484f-9b9e-de460746276c}",  # Internet Explorer 6 Language Pack
            ">{60B49E34-C7CC-11D0-8953-00A0C90347FF}",  # Microsoft Outlook Express
            "{2C7339CF-2B09-4501-B3F3-F3508C9228ED}",  # Windows Themes
            "{44BBA840-CC51-11CF-AAFA-00AA00B6015C}",  # Microsoft Outlook Express
            "{6BF52A52-394A-11d3-B153-00C04F79FAA6}",  # Windows Media Player
            "{7790769C-0471-11d2-AF11-00C04FA35D02}",  # Address Book 7 import tool
            "{89820200-ECBD-11cf-8B85-00AA005B4340}",  # Desktop Update
            "{6e801dad-cea5-4975-bdd3-6a97fc346bb2}"   # Windows Live Essentials
        )
        
        if (Test-Path $ActiveSetupPath) {
            $Components = Get-ChildItem -Path $ActiveSetupPath -ErrorAction SilentlyContinue
            
            foreach ($Component in $Components) {
                $ComponentId = $Component.PSChildName
                
                # Check if this component should be preserved
                if ($ComponentId -in $PreserveComponents) {
                    Write-Log "Preserving critical component: $ComponentId" "INFO"
                    $PreservedCount++
                    continue
                }
                
                # Check for additional preservation criteria
                $ComponentName = ""
                try {
                    $ComponentName = (Get-ItemProperty -Path $Component.PSPath -Name "(default)" -ErrorAction SilentlyContinue)."(default)"
                    if (-not $ComponentName) {
                        $ComponentName = (Get-ItemProperty -Path $Component.PSPath -Name "ComponentID" -ErrorAction SilentlyContinue)."ComponentID"
                    }
                } catch {
                    # Continue if we can't read the component name
                }
                
                # Preserve Microsoft and Windows critical components
                if ($ComponentName -match "(Microsoft|Windows|\.NET|DirectX|Visual C\+\+)" -and 
                    $ComponentName -notmatch "(Office|Word|Excel|PowerPoint|Outlook|Access|Publisher|Visio|Project|InfoPath)") {
                    Write-Log "Preserving Microsoft/Windows component: $ComponentId ($ComponentName)" "INFO"
                    $PreservedCount++
                    continue
                }
                ...
                try {
                    Remove-Item -Path $Component.PSPath -Recurse -Force -ErrorAction Stop
                    Write-Log "Removed Active Setup component: $ComponentId" "SUCCESS"
                    $RemovedCount++
                }
                catch {
                    Write-Log "Failed to remove component $ComponentId`: $($_.Exception.Message)" "WARN"
                }
            }
        }
        else {
            Write-Log "Active Setup path not found: $ActiveSetupPath" "WARN"
            return @{ Success = $false; RemovedCount = 0; PreservedCount = 0; Error = "Path not found" }
        }
        
        Write-Log "Active Setup cleanup completed: $RemovedCount removed, $PreservedCount preserved" "SUCCESS"
        return @{ 
            Success = $true
            RemovedCount = $RemovedCount
            PreservedCount = $PreservedCount
            TotalProcessed = $RemovedCount + $PreservedCount
        }
    }
    catch {
        Write-Log "Failed to clean Active Setup components: $($_.Exception.Message)" "ERROR"
        return @{ Success = $false; RemovedCount = 0; PreservedCount = 0; Error = $_.Exception.Message }
    }
}

function Disable-QuickAccessAndUserFolders {
    [CmdletBinding()]
    param()
    
    try {
        Write-Log "Disabling Quick Access and user folders..." "INFO"
        
        # Disable Quick Access
        $QuickAccessKey = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced"
        Set-ItemProperty -Path $QuickAccessKey -Name "LaunchTo" -Value 1 -Force
        
        # Hide user folders from File Explorer
        $ExplorerKey = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\FolderDescriptions"
        $UserFolders = @("Desktop", "Documents", "Downloads", "Music", "Pictures", "Videos")
        
        foreach ($Folder in $UserFolders) {
            $FolderPath = Join-Path $ExplorerKey $Folder
            if (Test-Path $FolderPath) {
                Set-ItemProperty -Path $FolderPath -Name "Attributes" -Value 1 -Force
            }
        }
        ...
        Write-Log "Failed to disable Quick Access: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Invoke-ProfileCleanup {
    ...
    try {
        Write-Log "Starting domain profile cleanup..." "INFO"
        ...
        foreach ($Profile in $ProfileList) {
            try {
                if ($Profile.LocalPath -notmatch "Administrator|Default") {
                    $Profile.Delete()
                    $CleanedProfiles++
                    Write-Log "Removed profile: $($Profile.LocalPath)" "SUCCESS"
                }
            }
            catch {
                Write-Log "Failed to remove profile $($Profile.LocalPath): $($_.Exception.Message)" "WARN"
            }
        }
        
        Write-Log "Profile cleanup completed: $CleanedProfiles profiles removed" "SUCCESS"
        return @{ Success = $true; CleanedProfiles = $CleanedProfiles }
    }
    catch {
        Write-Log "Profile cleanup failed: $($_.Exception.Message)" "ERROR"
        return @{ Success = $false; CleanedProfiles = 0 }
    }
}
...
function Invoke-SystemDefragmentation {
    ...
    try {
        Write-Log "Starting system defragmentation..." "INFO"
        
        # Check if drive is SSD (skip defrag for SSDs)
        $SystemDrive = $env:SystemDrive.Replace(":", "")
        $DriveType = Get-WmiObject -Class Win32_LogicalDisk | Where-Object { $_.DeviceID -eq "${SystemDrive}:" }
        
        if ($DriveType.DriveType -eq 3) {  # Fixed disk
            # Simple defrag command
            $DefragResult = Start-Process -FilePath "defrag" -ArgumentList "${SystemDrive}: /A" -Wait -PassThru
            ...
            if ($DefragResult.ExitCode -eq 0) {
                ...
                Write-Log "Defragmentation may not be needed or drive is SSD" "INFO"
                return @{ Success = $true; Skipped = $true }
            }
        }
        else {
            Write-Log "Skipping defragmentation - not a fixed disk" "INFO"
            return @{ Success = $true; Skipped = $true }
        }
    }
    catch {
        Write-Log "Defragmentation failed: $($_.Exception.Message)" "ERROR"
...
function Invoke-SystemOptimizations {
    ...
    try {
        ...
        $OptimizationResults = Set-RegistryOptimizations
        
        # Apply service optimizations
        $ServiceResults = Set-WindowsServices
        
        $OverallStatus = $OptimizationResults -and $ServiceResults
        
        Write-Log "System optimizations completed" "SUCCESS"
        return @{ OverallStatus = $OverallStatus; RegistryOptimizations = $OptimizationResults; ServiceOptimizations = $ServiceResults }
    }
    catch {
        Write-Log "System optimizations failed: $($_.Exception.Message)" "ERROR"
        return @{ OverallStatus = $false }
    }
}
...
function Invoke-VDIOptimizations {
    ...
    try {
        Write-Log "Applying VDI optimizations..." "INFO"
        
        # Start Citrix Optimizer if available
        $OptimizerResult = Start-CitrixOptimizer
        
        # Apply VDI-specific registry settings
        $VDIRegistryResult = Set-VDAMultipleMonitorHookKeys
        
        $OverallStatus = $OptimizerResult -or $VDIRegistryResult
        
        Write-Log "VDI optimizations completed" "SUCCESS"
        return @{ OverallStatus = $OverallStatus; Success = $OverallStatus; OptimizerResult = $OptimizerResult }
    }
    catch {
        Write-Log "VDI optimizations failed: $($_.Exception.Message)" "ERROR"
        return @{ OverallStatus = $false; Success = $false }
    }
}
...
function Invoke-WEMCleanup {
    ...
    try {
        Write-Log "Starting WEM RSA cleanup..." "INFO"
        
        # Remove WEM RSA keys
        $WEMResult = Remove-WEMRSAKey
        
        # Test cleanup success
        $CleanupTest = Test-WEMRSACleanup
        
        $Success = $WEMResult -and $CleanupTest
        
        Write-Log "WEM cleanup completed" "SUCCESS"
        return $Success
    }
    catch {
...
function Optimize-DotNetFramework {
    ...
    try {
        Write-Log "Optimizing .NET Framework..." "INFO"
        
        # Run .NET optimization
        $DotNetResult = Start-DotNetOptimization
        
        Write-Log ".NET Framework optimization completed" "SUCCESS"
        return @{ Success = $DotNetResult }
    }
    catch {
...
function Optimize-NetworkSettings {
    ...
    try {
        Write-Log "Optimizing network settings..." "INFO"
        ...
        $Success = $SMBResult -and $NetBiosResult -and $OffloadResult
...
function Optimize-StorageSettings {
    ...
    try {
        Write-Log "Optimizing storage settings..." "INFO"
        
        # Configure crash dump settings
        $CrashDumpResult = Set-CrashDumpToKernelMode
        
        # Start drive configuration
        $DriveConfigResult = Start-DriveConfiguration
        
        $Success = $CrashDumpResult -and $DriveConfigResult
        
        Write-Log "Storage optimizations completed" "SUCCESS"
        return @{ Success = $Success }
    }
    catch {
        Write-Log "Storage optimization failed: $($_.Exception.Message)" "ERROR"
        return @{ Success = $false }
    }
}

function Remove-PasswordAgeRegistry {
    [CmdletBinding()]
    param()
    
    try {
        Write-Log "Removing password age registry entries..." "INFO"
        
        $Result = Remove-PasswordAgeRegistryKey
...
function Remove-VirtualCacheDrives {
    [CmdletBinding()]
    param()
    
    try {
        Write-Log "Removing virtual cache drives..." "INFO"
        
        $Result = Remove-VirtualCacheDrive
        
        if ($Result) {
            Write-Log "Virtual cache drives removed" "SUCCESS"
            return @{ Success = $true; Skipped = $false }
        }
        else {
            Write-Log "No virtual cache drives found to remove" "INFO"
            return @{ Success = $true; Skipped = $true }
        }
    }
    catch {
    ...
    try {
        ...
        if ($InstallProcess.ExitCode -eq 0) {
            ...
            try {
                # Use Get-ItemProperty to properly handle registry paths with spaces
                try {
                    Get-ItemProperty -Path $UberAgentRegistryPath -ErrorAction Stop | Out-Null
                    $RegistryExists = $true
                } catch {
                    $RegistryExists = $false
                }
...
Export-ModuleMember -Function @(
   ...
   'Clear-ActiveSetupComponents', 'Disable-QuickAccessAndUserFolders', 'Invoke-ProfileCleanup', 'Invoke-SystemDefragmentation',
   'Invoke-SystemOptimizations', 'Invoke-VDIOptimizations', 'Invoke-WEMCleanup', 'Optimize-DotNetFramework',
   'Optimize-NetworkSettings', 'Optimize-StorageSettings', 'Remove-PasswordAgeRegistry', 'Remove-VirtualCacheDrives'
)