            ...
            try {
                ...
                if ($CDROMDrives) {
                    foreach ($Drive in $CDROMDrives) {
                        Write-Host "Found CD/DVD: $($Drive.Drive)" -ForegroundColor Yellow
                        if ($Drive.Drive -eq "D:") {
                            $IsCDDVD = $true
                            Write-Host "D: drive is CD/DVD ROM" -ForegroundColor Green
                            break
                        }
                    }
                }
                ...
                if (-not $IsCDDVD) {
                    try {
                        "test" | Out-File -FilePath "D:\test.tmp" -ErrorAction Stop
                        Remove-Item "D:\test.tmp" -Force -ErrorAction SilentlyContinue
                        Write-Host "D: drive is writable - not CD/DVD" -ForegroundColor White
                    } catch {
                        $IsCDDVD = $true
                        Write-Host "D: drive is read-only - assuming CD/DVD" -ForegroundColor Yellow
                    }
                }
