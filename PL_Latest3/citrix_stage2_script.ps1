...
try {
    ...
    # Critical: Domain profile cleanup
    Write-LogHeader "Domain Profile Cleanup"
    Write-Log "Removing domain user profiles to ensure clean OS layer..."
    $ProfileCleanupSuccess = Remove-DomainUserProfiles
    
    if ($ProfileCleanupSuccess.ProfilesRemoved -gt 0) {
        Write-Log "Domain profile cleanup completed: $($ProfileCleanupSuccess.ProfilesRemoved) profiles removed" "SUCCESS"
        foreach ($Profile in $ProfileCleanupSuccess.RemovedProfiles) {
            Write-Log "  Removed profile: $Profile" "SUCCESS"
        }
    }
    else {
        Write-Log "No domain profiles found to remove" "INFO"
    }
    ...
    if ($MaintenanceStatus -and $MaintenanceStatus.MaintenanceDisabled) {
        ...
        Write-Log "Consider disabling Automatic Maintenance for VDI environments" "WARN"
    }
