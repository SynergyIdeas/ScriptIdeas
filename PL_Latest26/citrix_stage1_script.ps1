...
if ($RelocateCDDVDDrive) {
    ...
    try {
        ...
        try {
            ...
            Write-Host "WMI/CIM not available - using fallback CD/DVD detection" -ForegroundColor Yellow
        ...
        if ($DDriveCheck -and $DDriveCheck.DriveType -eq 5) {
            ...
            $YDriveExists = $null
            try {
                $YDriveExists = Get-WmiOrCimInstance -ClassName Win32_LogicalDisk | Where-Object { $_.DeviceID -eq "Y:" }
            } catch {
                # Fallback check
            ...
            if (-not $YDriveExists) {
                ...
                try {
                    ...
                    $CDVolume = Get-WmiOrCimInstance -ClassName Win32_Volume | Where-Object { $_.DriveLetter -eq "D:" -and $_.DriveType -eq 5 }
                    if ($CDVolume) {
                        $CDVolume.DriveLetter = "Y:"
