...
if ($RelocateCDDVDDrive) {
    ...
    try {
        ...
        if (Test-Path "D:\") {
            ...
            try {
                Write-Host "Testing D: drive type using multiple detection methods..." -ForegroundColor Cyan
                
                # Method 1: Check drive type using WMI/CIM (most reliable)
                try {
                    $LogicalDisk = Get-CimInstance -ClassName Win32_LogicalDisk -Filter "DeviceID='D:'" -ErrorAction SilentlyContinue
                    if ($LogicalDisk) {
                        Write-Host "Drive type detected: $($LogicalDisk.DriveType)" -ForegroundColor Cyan
                        if ($LogicalDisk.DriveType -eq 5) {
                            $IsCDDVD = $true
                            Write-Host "D: drive detected as CD/DVD ROM (DriveType=5)" -ForegroundColor Yellow
                        } else {
                            Write-Host "D: drive is not CD/DVD ROM (DriveType=$($LogicalDisk.DriveType))" -ForegroundColor Green
                        }
                    } else {
                        Write-Host "Could not get drive information via CIM" -ForegroundColor Yellow
                    }
                } catch {
                    Write-Host "CIM detection failed: $($_.Exception.Message)" -ForegroundColor Yellow
                    ...
                    try {
                        ...
                        if ($LogicalDisk) {
                            Write-Host "WMI Drive type detected: $($LogicalDisk.DriveType)" -ForegroundColor Cyan
                            if ($LogicalDisk.DriveType -eq 5) {
                                $IsCDDVD = $true
                                Write-Host "D: drive detected as CD/DVD ROM via WMI (DriveType=5)" -ForegroundColor Yellow
                            } else {
                                Write-Host "D: drive is not CD/DVD ROM via WMI (DriveType=$($LogicalDisk.DriveType))" -ForegroundColor Green
                            }
                        }
                    } catch {
                        Write-Host "WMI detection also failed: $($_.Exception.Message)" -ForegroundColor Yellow
                    }
                }
                
                # Method 2: Check if drive is read-only by attempting to write (if WMI failed)
                if (-not $IsCDDVD) {
                    Write-Host "Testing write capability to D: drive..." -ForegroundColor Cyan
                    $TestFilePath = "D:\test_write_check.tmp"
                    try {
                        "test" | Out-File -FilePath $TestFilePath -ErrorAction Stop
                        Remove-Item $TestFilePath -Force -ErrorAction SilentlyContinue
                        Write-Host "D: drive is writable - not a CD/DVD ROM" -ForegroundColor Green
                    } catch {
                        $IsCDDVD = $true
                        Write-Host "D: drive is read-only - detected as CD/DVD ROM" -ForegroundColor Yellow
                    }
                }
                
                # Method 3: Check for CD/DVD volume characteristics (if still not detected)
                if (-not $IsCDDVD) {
                    try {
                        $Volume = Get-Volume -DriveLetter D -ErrorAction SilentlyContinue
                        if ($Volume) {
                            Write-Host "Volume FileSystem: $($Volume.FileSystem), Size: $($Volume.Size)" -ForegroundColor Cyan
                            # CD/DVD drives often have specific file systems
                            if ($Volume.FileSystem -eq "CDFS" -or $Volume.FileSystem -eq "UDF") {
                                $IsCDDVD = $true
                                Write-Host "D: drive has CD/DVD file system ($($Volume.FileSystem)) - detected as CD/DVD ROM" -ForegroundColor Yellow
                            }
                        }
                    } catch {
                        Write-Host "Volume detection failed: $($_.Exception.Message)" -ForegroundColor Yellow
                    }
                }
                
            } catch {
                Write-Host "All detection methods failed: $($_.Exception.Message)" -ForegroundColor Red
