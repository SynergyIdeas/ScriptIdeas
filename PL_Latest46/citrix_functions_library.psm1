...
"@
                    ...
                    if (-not $TargetDriveExists) {
                        ...
                        if ($DiskpartResult -match "successfully assigned" -or $DiskpartResult -match "completed successfully" -or -not ($DiskpartResult -match "error")) {
                            ...
                            Write-Log "Diskpart operation failed or returned errors" "ERROR"
                            Write-Log "Diskpart output: $DiskpartResult" "DEBUG"
                            $Results.Issues += "Failed to change CD/DVD ROM drive letter using diskpart"
                        }
                    } else {
...
function Get-WmiOrCimInstance {
    ...
    # Use WMI as fallback
    if ($Filter) {
        return Get-WmiOrCimInstance -ClassName $ClassName -Filter $Filter
    } else {
        return Get-WmiOrCimInstance -ClassName $ClassName
    }
}
...
function Set-PowerPlan {
    ...
    param(
        [Parameter(Mandatory=$false)]
        [string]$PlanName = "High performance"
    )
    
    try {
        Write-Log "Setting power plan to: $PlanName" "INFO"
        
        # Get power plan GUID
        $PowerPlan = Get-WmiOrCimInstance -ClassName Win32_PowerPlan -Namespace root\cimv2\power | Where-Object { $_.ElementName -like "*$PlanName*" }
        
        if ($PowerPlan) {
            try {
                # Use powercfg command instead of WMI methods
                $PowerPlanGuid = $PowerPlan.InstanceID.Split('\')[1]
                & powercfg /setactive $PowerPlanGuid
                Write-Log "Power plan set to: $($PowerPlan.ElementName) using powercfg" "SUCCESS"
                return $true
            } catch {
                Write-Log "Failed to set power plan using powercfg: $($_.Exception.Message)" "ERROR"
                return $false
            }
        } else {
            Write-Log "Power plan '$PlanName' not found" "ERROR"
            return $false
        }
    }
    catch {
...
function Disable-WindowsDefender {
    ...
    try {
        ...
        # Registry settings to disable Windows Defender
        $DefenderKeys = @(
        ...
        Write-Log "Failed to disable Windows Defender: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Disable-UAC {
    ...
    try {
        Write-Log "Disabling User Account Control (UAC)..." "INFO"
        
        Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" -Name "EnableLUA" -Value 0 -Force
        
        Write-Log "UAC disabled successfully" "SUCCESS"
        return $true
    }
    catch {
        Write-Log "Failed to disable UAC: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Enable-RDP {
    ...
    try {
        Write-Log "Enabling Remote Desktop Protocol (RDP)..." "INFO"
        ...
        Write-Log "RDP enabled successfully" "SUCCESS"
        return $true
    }
    catch {
        Write-Log "Failed to enable RDP: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Set-Autologon {
    ...
    try {
        Write-Log "Configuring autologon for user: $Username" "INFO"
        ...
        Set-ItemProperty -Path $AutoLogonPath -Name "DefaultUserName" -Value $Username -Force
        Set-ItemProperty -Path $AutoLogonPath -Name "DefaultPassword" -Value $Password -Force
        
        if (![string]::IsNullOrEmpty($Domain)) {
            Set-ItemProperty -Path $AutoLogonPath -Name "DefaultDomainName" -Value $Domain -Force
        }
        
        Write-Log "Autologon configured successfully" "SUCCESS"
        return $true
    }
    catch {
        Write-Log "Failed to configure autologon: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Install-CitrixVDA {
    ...
    try {
        ...
        if (Test-Path $VDAInstallerPath) {
            $Result = Install-VDAFromISO -ISOPath $VDAInstallerPath -ConfigFilePath $ConfigFilePath
            return $Result
        } else {
...
function Install-CitrixPVSTarget {
    ...
    try {
        ...
        if (Test-Path $PVSInstallerPath) {
            $Result = Install-PVSFromISO -ISOPath $PVSInstallerPath -ConfigFilePath $ConfigFilePath
            return $Result
        } else {
...
function Configure-IBMTADDMSCMPermissions {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$false)]
        [string]$ConfigFilePath = ".\CitrixConfig.txt"
...
function New-CacheDrive {
    ...
    try {
        Write-Log "Creating cache drive on $DriveLetter..." "INFO"
        
        # Check if drive already exists
        if (Test-Path "${DriveLetter}:\") {
            Write-Log "Drive ${DriveLetter}: already exists" "WARN"
            return @{
                Success = $true
                AlreadyExists = $true
        ...
        # Try to create virtual cache drive if enabled
        $UseVirtualCache = [bool](Get-ConfigValue -Key "UseVirtualCacheDrive" -DefaultValue "false" -ConfigFile $ConfigFilePath)
        if ($UseVirtualCache) {
            $VirtualResult = New-VirtualCacheDrive -DriveLetter $DriveLetter -VolumeLabel $VolumeLabel
            if ($VirtualResult.Success) {
                return $VirtualResult
            }
        }
        
        Write-Log "Physical cache drive must be attached manually" "WARN"
        return @{
            Success = $false
            RequiresManualAttachment = $true
            Error = "Physical drive attachment required"
        }
    }
    catch {
        Write-Log "Failed to create cache drive: $($_.Exception.Message)" "ERROR"
        return @{
            Success = $false
            Error = $_.Exception.Message
        }
...
function Set-VDAMultipleMonitorHookKeys {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$false)]
        [string]$ConfigFilePath = ".\CitrixConfig.txt"
    ...
    try {
        Write-Log "Configuring VDA multiple monitor hook registry keys..." "INFO"
        
        $HookKeys = @(
            @{ Path = "HKLM:\SOFTWARE\Citrix\Graphics"; Name = "UseMultipleMonitorHook"; Value = 1; Type = "DWORD" },
            @{ Path = "HKLM:\SOFTWARE\Citrix\Graphics"; Name = "MaxMonitors"; Value = 8; Type = "DWORD" },
            @{ Path = "HKLM:\SOFTWARE\Citrix\Graphics"; Name = "EnableMultipleMonitorSupport"; Value = 1; Type = "DWORD" }
        )
        
        $AppliedCount = 0
        foreach ($Key in $HookKeys) {
            if (Set-RegistryValue -Path $Key.Path -Name $Key.Name -Value $Key.Value -Type $Key.Type) {
                $AppliedCount++
                Write-Log "Applied: $($Key.Name)" "SUCCESS"
            }
        }
        
        Write-Log "VDA multiple monitor hook configuration completed: $AppliedCount keys applied" "SUCCESS"
        return @{
            Success = $true
            KeysApplied = $AppliedCount
        }
    }
    catch {
        Write-Log "Failed to configure VDA multiple monitor hooks: $($_.Exception.Message)" "ERROR"
        return @{
            Success = $false
            Error = $_.Exception.Message
        }
...
function New-VirtualCacheDrive {
    ...
    try {
        ...
        # Create VHDX file using diskpart (more compatible than New-VHD)
        Write-Log "Creating VHDX file: $CacheVHDXPath (${CacheDriveSizeMB}MB)" "INFO"
        
        try {
            # Windows diskpart method for VHDX creation
            $VHDXCreated = $false
            
            Write-Log "Creating VHDX using Windows diskpart" "INFO"
            try {
                $DiskpartScript = @"
...
active
format fs=ntfs label="Cache" quick
assign letter=$DriveLetter
"@
                $DiskpartScript | Out-File -FilePath "create_vhdx.txt" -Encoding ASCII
                $DiskpartResult = & "diskpart" /s create_vhdx.txt 2>&1
                Remove-Item "create_vhdx.txt" -Force -ErrorAction SilentlyContinue
                $VHDXCreated = $true
                Write-Log "VHDX created successfully using diskpart" "SUCCESS"
            } catch {
                Write-Log "Diskpart method failed: $($_.Exception.Message)" "WARN"
            }
            
            # Method 3: Fallback to directory-based cache simulation (Windows only)
            if (-not $VHDXCreated) {
                Write-Log "Creating directory-based cache as fallback" "INFO"
                
                # Windows cache directory creation
                $CacheBaseDir = "C:\CacheD"
                ...
                # Create organized subdirectories for different cache types
                $CacheSubDirs = @("PageFile", "EventLogs", "UserProfiles", "Temp", "AppData")
                foreach ($SubDir in $CacheSubDirs) {
                    $SubDirPath = Join-Path $CacheBaseDir $SubDir
                    if (-not (Test-Path $SubDirPath)) {
                        New-Item -ItemType Directory -Path $SubDirPath -Force | Out-Null
                    }
                }
                
                # Create a sparse file to simulate disk space usage
                $SparseFile = Join-Path $CacheBaseDir "cache_reserve.dat"
                try {
                    $FileStream = [System.IO.FileStream]::new($SparseFile, [System.IO.FileMode]::Create)
                    $FileStream.SetLength([int64]$CacheDriveSizeMB * 1MB)
                    $FileStream.Close()
                } catch {
                    # Ignore sparse file creation errors
                }
                
                $VHDXCreated = $true
                Write-Log "Directory-based cache created successfully at: $CacheBaseDir" "SUCCESS"
            }
            
            if (-not $VHDXCreated) {
                throw "All virtual drive creation methods failed"
            }
        } catch {
    ...
    try {
        ...
        try {
            ...
            if (-not (Test-Path $InstallerPath)) {
                ...
                $CommonPaths = @("", "x64", "x86", "Setup", "Installers")
            ...
            $InstallResult = @{
                ...
                ExitCode = $Process.ExitCode
                Duration = $Duration
                Error = if ($Process.ExitCode -eq 0 -or $Process.ExitCode -eq 3010) { $null } else { "Installation failed with exit code: $($Process.ExitCode)" }
            }
...
function Find-VDAInstaller {
    ...
    $PossiblePaths = @(
        ...
        "x64\Citrix\Virtual Desktop Agent",
        "Support\Additional"
    )
    
    foreach ($Path in $PossiblePaths) {
        foreach ($Name in $PossibleNames) {
            $FullPath = if ($Path) { Join-Path $MountPath $Path | Join-Path -ChildPath $Name } else { Join-Path $MountPath $Name }
            if (Test-Path $FullPath) {
                return $FullPath
            }
        }
...
function Find-PVSInstaller {
    param([string]$MountPath)
    
    $PossibleNames = @(
        "PVS_Device_x64.exe",
        "PVS_Device.exe", 
        "TargetDeviceSetup.exe",
        "PVSTargetDevice.exe",
        "BNDevice.exe"
    )
    ...
    foreach ($Path in $PossiblePaths) {
        foreach ($Name in $PossibleNames) {
            $FullPath = if ($Path) { 
                Join-Path $MountPath $Path | Join-Path -ChildPath $Name 
            } else { 
                Join-Path $MountPath $Name 
            }
            if (Test-Path $FullPath) {
                Write-Log "Found PVS installer: $FullPath" "SUCCESS"
                return $FullPath
            }
        }
    }
...
function Install-VDAFromISOWithDetection {
    ...
    try {
        ...
        try {
            ...
            Write-Log "Auto-detected VDA installer: $InstallerName at $InstallerPath" "SUCCESS"
            
            # Run the installation
            Write-Log "Starting $ProductName installation with arguments: $InstallerArgs" "INFO"
            $Process = Start-Process -FilePath $InstallerPath -ArgumentList $InstallerArgs -Wait -PassThru -NoNewWindow
            
            $ExitCode = $Process.ExitCode
            Write-Log "$ProductName installation completed with exit code: $ExitCode" "INFO"
            ...
            try {
                ...
                Write-Log "ISO dismounted successfully" "INFO"
            } catch {
                Write-Log "Warning: Failed to dismount ISO: $($_.Exception.Message)" "WARN"
            }
        }
    }
    catch {
        Write-Log "Failed to install $ProductName: $($_.Exception.Message)" "ERROR"
...
function Install-VDAFromISO {
    ...
    try {
        ...
        $VDAArgs = Get-ConfigValue -Key "VDAInstallArguments" -DefaultValue "/quiet /norestart /components vda,plugins /enable_hdx_ports /enable_real_time_transport /masterimage" -ConfigFile $ConfigFilePath
        
        # Install VDA from ISO using auto-detection within Install-FromISO
        $InstallResult = Install-VDAFromISOWithDetection -ISOPath $VDAISOPath -InstallerArgs $VDAArgs -ProductName "Citrix VDA" -TimeoutMinutes 45
...
function Install-PVSFromISO {
    ...
    try {
        ...
        # Mount ISO and auto-detect installer
        $MountResult = Mount-DiskImage -ImagePath $PVSISOPath -PassThru
        $DriveLetter = ($MountResult | Get-Volume).DriveLetter
        $MountPath = "${DriveLetter}:"
        
        try {
            # Auto-detect PVS installer
            $PVSInstallerPath = Find-PVSInstaller -MountPath $MountPath
            
            if (-not $PVSInstallerPath) {
                throw "No PVS installer found in ISO at any expected location"
            }
            
            $InstallerName = Split-Path -Leaf $PVSInstallerPath
            Write-Log "Auto-detected PVS installer: $InstallerName at $PVSInstallerPath" "SUCCESS"
            
            # Install PVS Target directly from detected installer
            Write-Log "Starting PVS Target installation with arguments: $PVSArgs" "INFO"
            $PVSProcess = Start-Process -FilePath $PVSInstallerPath -ArgumentList $PVSArgs -Wait -PassThru
            ...
            if ($Success) {
                $LogLevel = if ($ExitCode -eq 3010) { "WARN" } else { "SUCCESS" }
                $Message = if ($ExitCode -eq 3010) { "PVS Target installation completed successfully (reboot required)" } else { "PVS Target installation completed successfully" }
                Write-Log $Message $LogLevel
            } else {
                Write-Log "PVS Target installation failed with exit code: $ExitCode" "ERROR"
            }
            ...
            try {
                ...
                if ($ForceDelete) {
                    ...
                    try {
                        ...
                        $CmdResult = cmd /c "del /f /q `"$($File.FullName)`"" 2>&1
        ...
        foreach ($Folder in $Folders) {
            try {
                ...
                if ($ForceDelete) {
                    ...
                    try {
                        $CmdResult = cmd /c "rmdir /s /q `"$($Folder.FullName)`"" 2>&1
                        if ($LASTEXITCODE -eq 0) {
                            $result.FoldersRemoved++
                        } else {
...
function Add-CitrixVDA {
    ...
    try {
        ...
        # Auto-detect VDA installer
        $SetupPath = Find-VDAInstaller -MountPath $MountPath
        if ($SetupPath) {
            $InstallerName = Split-Path -Leaf $SetupPath
            Write-Log "Auto-detected VDA installer: $InstallerName at $SetupPath" "SUCCESS"
            ...
            Write-Log "No VDA installer found in ISO at any expected location" "ERROR"
            Write-Log "Searched paths: root, x64, x64\\XenDesktop Setup, x64\\Citrix\\Virtual Desktop Agent, Support\\Additional" "ERROR"
            Write-Log "Expected installer names: VDAWorkstationSetup.exe, VDAServerSetup.exe, VDAWorkstationCoreSetup.exe, VDAServerCoreSetup.exe, XenDesktopVdaSetup.exe, CitrixVDA.exe" "ERROR"
            $InstallSuccess = $false
        }
...
function Add-PVSTargetDevice {
    ...
    try {
        ...
        # Auto-detect PVS installer
        $SetupPath = Find-PVSInstaller -MountPath $MountPath
        if ($SetupPath) {
            $InstallerName = Split-Path -Leaf $SetupPath
            Write-Log "Auto-detected PVS installer: $InstallerName at $SetupPath" "SUCCESS"
            ...
            if ($PVSProcess.ExitCode -eq 0 -or $PVSProcess.ExitCode -eq 3010) {
                $LogLevel = if ($PVSProcess.ExitCode -eq 3010) { "WARN" } else { "SUCCESS" }
                $Message = if ($PVSProcess.ExitCode -eq 3010) { "PVS Target Device installation completed successfully (reboot required)" } else { "PVS Target Device installation completed successfully" }
                Write-Log $Message $LogLevel
                $InstallSuccess = $true
            } else {
            ...
            Write-Log "No PVS installer found in ISO at any expected location" "ERROR"
            Write-Log "Searched paths: Device, Target Device, x64\\Device, Installers, root" "ERROR"
            Write-Log "Expected installer names: PVS_Device_x64.exe, PVS_Device.exe, TargetDeviceSetup.exe, PVSTargetDevice.exe, BNDevice.exe" "ERROR"
            $InstallSuccess = $false
        }
    ...
    try {
        ...
        if ($InstallProcess.ExitCode -eq 0) {
            ...
            try {
                ...
                if (Test-Path $UberAgentRegPath) {
                    Write-Log "UberAgent registry key found - validating installation..." "SUCCESS"
                    
                    # Delete the entire registry key for template preparation
                    Remove-Item -Path $UberAgentRegPath -Recurse -Force -ErrorAction Stop
                    Write-Log "UberAgent registry key deleted for template preparation" "SUCCESS"
                    $Results.RegistryCleared = $true
                } else {
                    $ErrorMsg = "UberAgent registry key not found at: $UberAgentRegPath - installation may have failed"
            ...
            try {
                if (Test-Path $UberAgentRegistryPath) {
...
# Export all functions automatically to ensure complete compatibility
...
$AllFunctions = @(
    ...
    'Get-CacheDrive', 'New-CacheDrive', 'Set-PageFile', 'Clear-EventLogs', 'Set-PowerPlan', 'Disable-WindowsUpdates',
    'Disable-WindowsDefender', 'Disable-UAC', 'Enable-RDP', 'Set-TimeZone', 'Join-Domain', 'Set-LocalUserPassword',
    'Set-Autologon', 'Install-CitrixVDA', 'Install-CitrixPVSTarget', 'Install-CitrixWEMAgent', 'Install-UberAgent',
    'Install-IBMTADDMAgent', 'Configure-IBMTADDMSCMPermissions', 'Start-CitrixOptimizer', 'Install-CitrixOptimizer', 'Optimize-CitrixVDI',
    ...
    'Test-SystemOptimizations', 'Test-WEMRSACleanup', 'Validate-InstallationFiles', 'Get-WmiOrCimInstance', 'Test-SafePath',
    'Invoke-WithRetry', 'Test-Configuration', 'Write-ProgressHelper', 'Get-SecureCredential', 'Test-ValidInput', 'Copy-FileWithValidation'
)
