    ...
    try {
        ...
        try {
            ...
            if (Get-Command "New-VHD" -ErrorAction SilentlyContinue) {
                try {
                    # Try different parameter names for different PowerShell versions
                    $SizeInBytes = [int64]$CacheDriveSizeMB * 1MB
                    
                    # Try SizeBytes parameter first (newer versions)
                    try {
                        New-VHD -Path $CacheVHDXPath -SizeBytes $SizeInBytes -Dynamic
                    } catch {
                        # Fallback to Size parameter (older versions)
                        New-VHD -Path $CacheVHDXPath -Size $SizeInBytes -Dynamic
                    }
