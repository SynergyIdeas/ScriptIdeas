            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        Write-Host "WMI/CIM detection failed - using alternative methods" -ForegroundColor Yellow
                    }
                }
                
                # Method 2: Check if drive is read-only by attempting to write
                if (-not $IsCDDVD) {
                    ...
                    try {
                        ...
                        $IsCDDVD = $true
                        Write-Host "D: drive is read-only - likely CD/DVD ROM" -ForegroundColor Yellow
                    }
                }
                
                # Method 3: Check for typical CD/DVD file system characteristics
                if (-not $IsCDDVD) {
                    try {
                        $DriveInfo = Get-PSDrive -Name "D" -ErrorAction SilentlyContinue
                        if ($DriveInfo) {
                            # CD/DVD drives often show 0 or very specific sizes
                            if ($DriveInfo.Used -eq 0 -and $DriveInfo.Free -eq 0) {
                                $IsCDDVD = $true
                                Write-Host "D: drive shows empty usage - likely CD/DVD ROM" -ForegroundColor Yellow
                            }
                        }
                    } catch {
                ...
                Write-Host "Could not determine D: drive type - assuming it's a regular drive" -ForegroundColor Yellow
            }
