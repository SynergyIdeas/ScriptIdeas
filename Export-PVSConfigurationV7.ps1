...
param (
    ...
    [string]$OutputFolder = $(Join-Path -Path "." -ChildPath "PVSExport_$(Get-Date -Format 'yyyyMMdd_HHmmss')"),
    
    [Parameter(Mandatory=$false)]
    [string]$LogFile = $(Join-Path -Path "." -ChildPath "PVSExport_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"),
    ...
    try {
        ...
        if ($vDisks) {
            ...
            foreach ($storeGroup in $vDisksByStore) {
                ...
                $storeVDisksFile = Join-Path -Path $vDiskFolder -ChildPath "vDisks_Store_${storeName}.xml"
