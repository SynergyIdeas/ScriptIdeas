...
function Configure-NTPTimeSources {
    ...
    try {
        ...
        try {
            ...
            Set-ItemProperty -Path $W32TimeConfigPath -Name "NtpServer" -Value ($NTPServerArray -join ',0x1 ') + ',0x1'
...
function Initialize-CitrixPowerShell {
    [CmdletBinding()]
    param()
    
    try {
        Write-Log "Initializing Citrix PowerShell environment..." "INFO"
        ...
        # Load Citrix PowerShell snapins
        try {
            $CitrixSnapins = Get-PSSnapin -Registered | Where-Object { $_.Name -like "Citrix.*" }
            
            if ($CitrixSnapins) {
                foreach ($Snapin in $CitrixSnapins) {
                    try {
                        if (-not (Get-PSSnapin -Name $Snapin.Name -ErrorAction SilentlyContinue)) {
                            Add-PSSnapin -Name $Snapin.Name
                            $Results.SnapinsLoaded += $Snapin.Name
                            Write-Log "Loaded Citrix PowerShell snapin: $($Snapin.Name)" "SUCCESS"
                        } else {
                            Write-Log "Citrix PowerShell snapin already loaded: $($Snapin.Name)" "INFO"
                            $Results.SnapinsLoaded += $Snapin.Name
                        }
                    }
                    catch {
                        $Results.Errors += "Failed to load snapin $($Snapin.Name): $($_.Exception.Message)"
                        Write-Log "Failed to load Citrix snapin $($Snapin.Name): $($_.Exception.Message)" "ERROR"
                    }
                }
            } else {
                Write-Log "No Citrix PowerShell snapins found on system" "WARNING"
            }
        }
        catch {
            $Results.Errors += "Error loading Citrix snapins: $($_.Exception.Message)"
            Write-Log "Error loading Citrix snapins: $($_.Exception.Message)" "ERROR"
        }
        
        # Disable Citrix Call Home if cmdlet is available
        try {
            if (Get-Command "Disable-CitrixCallHome" -ErrorAction SilentlyContinue) {
                Disable-CitrixCallHome
                $Results.TelemetryDisabled = $true
                Write-Log "Disabled Citrix Call Home telemetry" "SUCCESS"
            } else {
                Write-Log "Disable-CitrixCallHome cmdlet not available" "WARNING"
            }
        }
        catch {
            $Results.Errors += "Failed to disable Citrix Call Home: $($_.Exception.Message)"
            Write-Log "Failed to disable Citrix Call Home: $($_.Exception.Message)" "ERROR"
        }
        
        # Disable CitrixTelemetryService
        try {
            $TelemetryService = Get-Service -Name "CitrixTelemetryService" -ErrorAction SilentlyContinue
            if ($TelemetryService) {
                if ($TelemetryService.Status -eq "Running") {
                    Stop-Service -Name "CitrixTelemetryService" -Force
                    Write-Log "Stopped CitrixTelemetryService" "SUCCESS"
                }
                Set-Service -Name "CitrixTelemetryService" -StartupType Disabled
                $Results.ServiceDisabled = $true
                Write-Log "Disabled CitrixTelemetryService startup" "SUCCESS"
            } else {
                Write-Log "CitrixTelemetryService not found on system" "INFO"
            }
        }
        catch {
            $Results.Errors += "Failed to disable CitrixTelemetryService: $($_.Exception.Message)"
            Write-Log "Failed to disable CitrixTelemetryService: $($_.Exception.Message)" "ERROR"
        }
        
        # Set registry to disable CEIP telemetry
        try {
            $TelemetryRegPath = "HKLM:\SOFTWARE\Citrix\Telemetry\CEIP"
            if (-not (Test-Path $TelemetryRegPath)) {
                New-Item -Path $TelemetryRegPath -Force | Out-Null
                Write-Log "Created Citrix telemetry registry path" "INFO"
            }
            Set-ItemProperty -Path $TelemetryRegPath -Name "Enabled" -Value 0 -Type DWord
            $Results.RegistrySet = $true
            Write-Log "Disabled Citrix CEIP telemetry in registry" "SUCCESS"
        }
        catch {
            $Results.Errors += "Failed to set CEIP registry: $($_.Exception.Message)"
            Write-Log "Failed to set Citrix CEIP registry: $($_.Exception.Message)" "ERROR"
        }
        
        # Set overall success based on critical operations
        if ($Results.Errors.Count -gt 0) {
            $Results.Success = $false
        }
        
        $SnapinCount = $Results.SnapinsLoaded.Count
        Write-Log "Citrix PowerShell initialization completed - $SnapinCount snapins loaded, telemetry disabled" "INFO"
        
        return $Results
    }
    catch {
        Write-Log "Critical error during Citrix PowerShell initialization: $($_.Exception.Message)" "ERROR"
        return @{
            Success = $false
            SnapinsLoaded = @()
            TelemetryDisabled = $false
            ServiceDisabled = $false
            RegistrySet = $false
            Errors = @("Critical initialization error: $($_.Exception.Message)")
        }
    }
}
...
Export-ModuleMember -Function @(
   ...
   'Add-CitrixVDA', 'Add-Domain', 'Add-PVSTargetDevice', 'Add-StartupShutdownScripts', 'Clear-WindowsEventLogs', 'Initialize-CitrixPowerShell', 'Configure-NTPTimeSources',
