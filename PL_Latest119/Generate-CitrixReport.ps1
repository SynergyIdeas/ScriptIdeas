function Get-ComponentDetails {
    ...
    switch ($ComponentName) {
        ...
        "Citrix PowerShell Initialization" {
            if ($Result.Success) {
                $detailsHTML += '<div class="detail-section">'
                $detailsHTML += '<div class="detail-section-title">Citrix PowerShell Environment</div>'
                $detailsHTML += '<div class="detail-item detail-success">[SUCCESS] Citrix PowerShell environment initialized</div>'
                if ($Result.SnapinsLoaded -and $Result.SnapinsLoaded.Count -gt 0) {
                    $detailsHTML += "<div class='detail-item detail-success'>PowerShell snapins loaded: $($Result.SnapinsLoaded.Count)</div>"
                    foreach ($snapin in $Result.SnapinsLoaded) {
                        $detailsHTML += "<div class='detail-item detail-info'>  • $snapin</div>"
                    }
                }
                $detailsHTML += '</div>'
                
                $detailsHTML += '<div class="detail-section">'
                $detailsHTML += '<div class="detail-section-title">Telemetry Disabling</div>'
                if ($Result.TelemetryDisabled) {
                    $detailsHTML += '<div class="detail-item detail-success">[SUCCESS] Citrix Call Home telemetry disabled</div>'
                }
                if ($Result.ServiceDisabled) {
                    $detailsHTML += '<div class="detail-item detail-success">[SUCCESS] CitrixTelemetryService disabled</div>'
                }
                if ($Result.RegistrySet) {
                    $detailsHTML += '<div class="detail-item detail-success">[SUCCESS] CEIP telemetry registry disabled</div>'
                }
                $detailsHTML += '</div>'
            } else {
                $detailsHTML += '<div class="detail-section">'
                $detailsHTML += '<div class="detail-section-title">Citrix PowerShell Initialization Issues</div>'
                $detailsHTML += '<div class="detail-item detail-error">[ERROR] Citrix PowerShell initialization encountered issues</div>'
                if ($Result.Errors -and $Result.Errors.Count -gt 0) {
                    foreach ($error in $Result.Errors) {
                        $detailsHTML += "<div class='detail-item detail-error'>Error: $error</div>"
                    }
...
function New-CitrixReport {
    ...
    $ComponentDisplayNames = @{
        ...
        'Citrix PowerShell Initialization' = 'Citrix PowerShell Environment'
