...
function Write-Log {
    ...
    # File logging - ensure log file is created
    if ($Global:LogPath) {
        try {
...
function Set-PowerPlan {
    ...
    try {
        ...
        # Get power plan GUID
        $PowerPlan = Get-WmiObject -Namespace root\cimv2\power -Class Win32_PowerPlan | Where-Object { $_.ElementName -like "*$PlanName*" }
        
        if ($PowerPlan) {
            $PowerPlan.Activate()
            Write-Log "Power plan set to: $($PowerPlan.ElementName)" "SUCCESS"
            return $true
        } else {
...
function Configure-IBMTADDMSCMPermissions {
    ...
    try {
        ...
        if ($EnableSCMConfig) {
            ...
            $Result = Set-IBMTADDMSCMPermissions -SCMPath $SCMPath
            return $Result
        } else {
...
function Get-CacheDrive {
    ...
    try {
        Write-Log "Validating cache drive $DriveLetter..." "INFO"
        
        # Check if drive exists
        if (-not (Test-Path "${DriveLetter}:\")) {
            Write-Log "Cache drive ${DriveLetter}: not found" "ERROR"
            return $false
        }
        ...
        if (-not $DriveInfo) {
            Write-Log "Unable to get drive information for ${DriveLetter}:" "ERROR"
            return $false
        }
        
        # Validate drive type (3 = Fixed disk, not CD/DVD)
        if ($DriveInfo.DriveType -ne 3) {
            Write-Log "Drive ${DriveLetter}: is not a fixed disk (Type: $($DriveInfo.DriveType))" "ERROR"
            return $false
        }
        
        # Check minimum space requirement
        $MinSpaceGB = [int](Get-ConfigValue -Key "CacheDriveMinSpaceGB" -DefaultValue "10" -ConfigFile $ConfigFilePath)
        $FreeSpaceGB = [math]::Round($DriveInfo.FreeSpace / 1GB, 2)
        
        if ($FreeSpaceGB -lt $MinSpaceGB) {
            Write-Log "Cache drive has insufficient space: ${FreeSpaceGB}GB (minimum: ${MinSpaceGB}GB)" "ERROR"
            return $false
        }
        
        Write-Log "Cache drive validated successfully: ${FreeSpaceGB}GB free space" "SUCCESS"
        return $true
    }
    catch {
        Write-Log "Cache drive validation failed: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Copy-FileWithValidation {
    ...
    try {
        ...
        if (-not (Test-SafePath -Path $SourcePath)) {
            throw "Source file not found or inaccessible: $SourcePath"
        }
        
        # Create destination directory if needed
        $DestDir = Split-Path $DestinationPath -Parent
        if (-not (Test-SafePath -Path $DestDir -CreateIfMissing)) {
            throw "Cannot create destination directory: $DestDir"
        }
        ...
        if (-not (Test-Path $DestinationPath)) {
            throw "Copy operation failed - destination file not found"
        }
        
        Write-Log "File copied successfully: $([System.IO.Path]::GetFileName($SourcePath))" "SUCCESS"
        return $true
    }
    catch {
...
function Validate-InstallationFiles {
    ...
    try {
        Write-Log "Validating installation files..." "INFO"
        ...
        $FilesToValidate = @{
            "VDA ISO" = Get-ConfigValue -Key "VDAISOSourcePath" -ConfigFile $ConfigFilePath
            "PVS ISO" = Get-ConfigValue -Key "PVSISOSourcePath" -ConfigFile $ConfigFilePath  
            "WEM Agent" = Get-ConfigValue -Key "WEMAgentSource" -ConfigFile $ConfigFilePath
            "UberAgent" = Get-ConfigValue -Key "UberAgentSource" -ConfigFile $ConfigFilePath
        }
        
        foreach ($FileInfo in $FilesToValidate.GetEnumerator()) {
            $ValidationResults.TotalChecked++
            $FilePath = $FileInfo.Value
            
            if ([string]::IsNullOrWhiteSpace($FilePath)) {
                Write-Log "Skipping validation for $($FileInfo.Key) - not configured" "INFO"
                continue
            }
            
            if (-not (Test-SafePath -Path $FilePath)) {
                $ValidationResults.Valid = $false
                $ValidationResults.InaccessibleFiles += "$($FileInfo.Key): $FilePath"
                Write-Log "File not accessible: $($FileInfo.Key) at $FilePath" "ERROR"
            } else {
                Write-Log "File validated: $($FileInfo.Key)" "SUCCESS"
            }
        }
        
        Write-Log "File validation completed: $($ValidationResults.TotalChecked) files checked" "INFO"
        return $ValidationResults
    }
    catch {
        Write-Log "File validation failed: $($_.Exception.Message)" "ERROR"
...
function Install-VDAFromISO {
    ...
    try {
        ...
        $InstallResult = Install-FromISO -ISOPath $VDAISOPath -InstallerName "VDAServerSetup.exe" -InstallerArgs $VDAArgs -ProductName "Citrix VDA" -TimeoutMinutes 45
...
function Install-PVSFromISO {
    ...
    try {
        ...
        $InstallResult = Install-FromISO -ISOPath $PVSISOPath -InstallerName "PVS_Target.exe" -InstallerArgs $PVSArgs -ProductName "PVS Target Device" -TimeoutMinutes 20
...
function Start-SystemDriveDefragmentation {
    ...
    try {
        ...
        try {
            $DriveInfo = Get-WmiObject -Class Win32_LogicalDisk -Filter "DeviceID='$DriveToProcess'"
        ...
        try {
            ...
            while ($OperationDetected -and ((Get-Date) - $WaitStartTime).TotalMinutes -lt $MaxWaitMinutes) {
                ...
                try {
                    $VolumeOptJobs = Get-WmiObject -Class Win32_Process -Filter "CommandLine LIKE '%Optimize-Volume%' OR CommandLine LIKE '%StorageWMI%'" -ErrorAction SilentlyContinue
                    if ($VolumeOptJobs) {
                        $OperationDetected = $true
        ...
        if ($DefragResults.Success -and $DefragResults.DefragmentationPerformed) {
            try {
                ...
                $PostDefragInfo = Get-WmiObject -Class Win32_LogicalDisk -Filter "DeviceID='$DriveToProcess'"
                ...
                if ($GhostDevices.Count -gt 0) {
                    ...
                    foreach ($Device in $GhostDevices) {
                        try {
                            ...
                            # Use WMI to remove the device
                            $Device.Delete()
                            $RemovalCount++
                            Write-Log "Removed ghost device: $($Device.Name)" "SUCCESS"
                        }
                        catch {
                            $FailureCount++
...
"@
            ...
            # Environment-aware script path creation
            $TempScript = if ($IsWindows -or $env:OS -eq "Windows_NT") {
                "$env:TEMP\create_virtual_cache.txt"
            } else {
                "/tmp/create_virtual_cache.txt"
            }
            
            try {
                $DiskPartScript | Out-File -FilePath $TempScript -Encoding ASCII -Force
            } catch {
                Write-Log "Cannot create diskpart script in Linux environment: $($_.Exception.Message)" "WARNING"
                Write-Log "Virtual cache drive creation not supported in this environment" "WARNING"
                return @{
                    Success = $false
                    Error = "Virtual cache drive creation requires Windows environment"
                    DriveLetter = $null
                }
            }
            ...
            if ($DiskPartProcess.ExitCode -eq 0) {
                ...
                # Try Hyper-V method as fallback if available (Windows only)
                if (($IsWindows -or $env:OS -eq "Windows_NT") -and (Get-Command "New-VHD" -ErrorAction SilentlyContinue)) {
                    Write-Log "Attempting Hyper-V fallback method..." "INFO"
                    
                    try {
                        $NewVHD = New-VHD -Path $VHDXPath -Size ($SizeMB * 1MB) -Dynamic
                    ...
                    if ($IsWindows -or $env:OS -eq "Windows_NT") {
                        ...
                        Write-Log "Virtual cache drive creation not supported in non-Windows environments" "INFO"
                    }
                }
            ...
            if ($IsWindows -or $env:OS -eq "Windows_NT") {
                ...
                throw "New-VHD cmdlet not available in this environment"
            }
