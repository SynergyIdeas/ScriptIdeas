...
try {
    ...
    # Readiness assessment moved to after cache drive removal
    $ReadinessScore = 0
    $MaxScore = 23
    $ReadinessIssues = @()
    ...
    # 23. Active Components Registry Cleanup
    if ($ActiveComponentsResult -and ($ActiveComponentsResult.Success -or $ActiveComponentsResult.TotalRemoved -ge 0)) {
        $ReadinessScore += 1
        Write-Log "23. Active Components Registry: COMPLETED" "SUCCESS"
    } else {
        $ReadinessIssues += "Active Components registry cleanup incomplete"
        Write-Log "23. Active Components Registry: INCOMPLETE" "WARN"
    }
