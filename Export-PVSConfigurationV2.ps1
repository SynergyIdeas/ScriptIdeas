...
try {
    ...
    foreach ($farm in $pvsFarmList) {
        ...
        try {
            ...
            $siteCount = ($pvsSites | Measure-Object).Count
            Write-Log "Found $siteCount site(s) in farm $farmName" "INFO"
            
            if ($siteCount -eq 0) {
                Write-Log "No sites found in farm $farmName. Please check PVS configuration." "WARNING"
                continue
            }
        }
        catch {
            Write-Log "Error retrieving sites for farm $farmName: $($_.Exception.Message)" "ERROR"
            continue
        }
