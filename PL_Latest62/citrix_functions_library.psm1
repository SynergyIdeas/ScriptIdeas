...
function Get-ConfigValue {
    ...
    try {
        ...
        if ($ConfigData.ContainsKey($Key)) {
            ...
            $MaxExpansionAttempts = 10  # Prevent infinite loops
            $ExpansionAttempts = 0
            
            while ($Value -match '%([^%]+)%' -and $ExpansionAttempts -lt $MaxExpansionAttempts) {
                $VarName = $Matches[1]
                $VarValue = $null
                $ExpansionAttempts++
                ...
                if ($VarValue) {
                    ...
                    # For unresolved variables, keep them as-is instead of removing
                    # This prevents breaking paths and makes debugging easier
                    Write-Verbose "Warning: Cannot resolve variable '$VarName' in value '$Value'"
                    break  # Stop expansion to prevent infinite loops
                }
            }
            
            # Final check for any remaining unresolved variables
            if ($Value -match '%([^%]+)%') {
                $UnresolvedVars = ([regex]'%([^%]+)%').Matches($Value) | ForEach-Object { $_.Groups[1].Value }
                Write-Verbose "Unresolved variables in '$Key': $($UnresolvedVars -join ', ')"
            }
...
function Copy-AllInstallationFiles {
    ...
    try {
        ...
        foreach ($FileInfo in $FilesToCopy.GetEnumerator()) {
            ...
            # Check source exists with enhanced diagnostics
            if (-not (Test-Path $SourcePath)) {
                # Provide detailed diagnostics for missing source files
                $ErrorDetails = "Source file not found: $SourcePath"
                
                # Check if path contains unresolved variables
                if ($SourcePath -match '%([^%]+)%') {
                    $UnresolvedVars = ([regex]'%([^%]+)%').Matches($SourcePath) | ForEach-Object { $_.Groups[1].Value }
                    $ErrorDetails += "`n  Unresolved variables detected: $($UnresolvedVars -join ', ')"
                    $ErrorDetails += "`n  Check configuration file for missing variable definitions"
                }
                
                # Check if it's a network path
                if ($SourcePath -like "\\*") {
                    $ErrorDetails += "`n  Network path - verify connectivity and permissions"
                    $ParentPath = Split-Path $SourcePath -Parent
                    if (-not [string]::IsNullOrEmpty($ParentPath) -and (Test-Path $ParentPath)) {
                        $ErrorDetails += "`n  Parent directory exists but file is missing"
                    } else {
                        $ErrorDetails += "`n  Parent directory is also inaccessible"
                    }
                }
            ...
            if (![string]::IsNullOrEmpty($LicenseSourcePath)) {
                ...
                if (Test-Path $LicenseSourcePath) {
                    try {
                        ...
                        # Check if license file already exists and compare with source
                        ...
                        if ($LicenseExists) {
                            ...
                            $ExistingFile = Get-Item $LicenseLocalPath
                            $ExistingSize = $ExistingFile.Length
                            $ExistingHash = (Get-FileHash -Path $LicenseLocalPath -Algorithm MD5).Hash
                            
                            if ($SourceSize -eq $ExistingSize -and $SourceHash -eq $ExistingHash) {
                                Write-Log "UberAgent license file is identical to source - skipping copy" "INFO"
                                $SkipCopy = $true
                                $CopyAction = "SKIPPED"
                            } else {
                                Write-Log "UberAgent license file differs from source - will overwrite" "INFO"
                        ...
                        if (-not $SkipCopy) {
                            if ($LicenseExists) {
                                Remove-Item -Path $LicenseLocalPath -Force -ErrorAction SilentlyContinue
                            }
                            Copy-Item -Path $LicenseSourcePath -Destination $LicenseLocalPath -Force
                        }
                        
                        # Validate license file (whether copied or skipped)
                        if (Test-Path $LicenseLocalPath) {
                            if ($SkipCopy) {
                                # File was skipped because it's identical
                                Write-Log "[$CopyAction] UberAgent license file already matches source ($([Math]::Round($SourceSize/1KB, 1)) KB)" "SUCCESS"
                                $Results.LicenseCopied = $true
                                $Results.FilesProcessed += "License: uberagent.lic [$CopyAction]"
                            } else {
                                # File was copied, validate it
                                ...
                                if ($SourceSize -eq $DestSize -and $SourceHash -eq $DestHash) {
                                    ...
                                    if ($SizeMismatch) {
                                        Write-Log "License file size mismatch - Source: $SourceSize bytes, Destination: $DestSize bytes" "ERROR"
                                    }
                                    if ($HashMismatch) {
                                        Write-Log "License file hash mismatch - Source: $SourceHash, Destination: $DestHash" "ERROR"
                                    }
                                    
                                    $ErrorMsg = "License file validation failed - Size/Hash mismatch (Source: $([Math]::Round($SourceSize/1KB, 1)) KB, Dest: $([Math]::Round($DestSize/1KB, 1)) KB)"
                                    Write-Log $ErrorMsg "ERROR"
                                    $Results.Errors += $ErrorMsg
                                }
                            }
