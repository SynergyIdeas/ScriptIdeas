    ...
    switch ($ComponentName) {
        ...
        "WEM RSA Cleanup" {
            if ($Result.Success) {
                ...
                if ($Result.NoFilesFound) {
                    ...
                    $totalFound = ($Result.RemovedKeys.Count + $Result.FailedRemovals.Count)
                    $detailsHTML += "<div class='detail-item detail-info'>Files found: $totalFound</div>"
                    $detailsHTML += "<div class='detail-item detail-success'>Successfully removed: $($Result.RemovedKeys.Count)</div>"
                    if ($Result.FailedRemovals.Count -gt 0) {
                        $detailsHTML += "<div class='detail-item detail-warning'>Failed removals: $($Result.FailedRemovals.Count)</div>"
                    }
                }
                $detailsHTML += '</div>'
        ...
        "Installed Software Versions" {
            if ($Result.Success) {
                ...
                if ($Result.TADDMVersion) {
                    $detailsHTML += "<div class='detail-item detail-success'>IBM TADDM: $($Result.TADDMVersion)</div>"
                }
...
function New-CitrixReport {
    ...
    foreach ($ComponentName in $SortedComponents) {
        ...
        if ($Result -is [hashtable]) {
            ...
            # Check for skip-related error messages (matching console logic exactly)
            ...
            # Check for skip-related messages (matching console logic exactly)
            ...
            if ($ComponentName -eq ".NET Framework Optimization" -and $Result.Error -match "ngen|not found") {
                $isSkipped = $true
            }
            
            if ($ComponentName -eq "System Defragmentation" -and $Result.Error -match "disabled|skipped") {
                $isSkipped = $true
            }
            
            if ($ComponentName -eq "UberAgent Service Configuration" -and $Result.Error -match "not found|not configured") {
                $isSkipped = $true
            }
            
            if ($ComponentName -eq "Event Logs Cleanup" -and $Result.Error -match "disabled|not configured") {
                $isSkipped = $true
            }
            ...
            if ($ComponentName -eq "NTP Time Configuration" -and $Result.Error -match "disabled|not configured") {
                $isSkipped = $true
            }
