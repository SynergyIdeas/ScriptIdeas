...
function Get-InstalledSoftwareVersions {
    ...
    try {
        ...
        # Check WEM Agent version
        try {
            $WEMPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*"
            $WEMProduct = Get-ItemProperty $WEMPath -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -like "*Workspace Environment Management*" -or $_.DisplayName -like "*WEM*Agent*" }
            if ($WEMProduct) {
        ...
        # Check IBM TADDM version
        try {
            $TADDMPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*"
            $TADDMProduct = Get-ItemProperty $TADDMPath -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -like "*IBM*TADDM*" -or $_.DisplayName -like "*Tivoli*" }
            if ($TADDMProduct) {
                $Results.TADDMVersion = $TADDMProduct.DisplayVersion
                $Results.Details += "IBM TADDM version: $($TADDMProduct.DisplayVersion)"
                Write-Log "Found IBM TADDM version: $($TADDMProduct.DisplayVersion)" "SUCCESS"
            } else {
                $Results.Details += "IBM TADDM: Not installed"
                Write-Log "IBM TADDM not found in registry" "INFO"
            }
        } catch {
            $Results.Errors += "Failed to check TADDM version: $($_.Exception.Message)"
        }
        ...
        $FoundSoftware = @($Results.VDAVersion, $Results.PVSVersion, $Results.WEMVersion, $Results.TADDMVersion, $Results.UberAgentVersion) | Where-Object { -not [string]::IsNullOrEmpty($_) }
