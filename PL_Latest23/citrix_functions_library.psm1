            ...
            if ($DDriveInfo.DriveType -eq 5) {
                ...
                try {
                    ...
                    if ($CDDrive) {
                        ...
                        $TargetDriveExists = Get-CimInstance -ClassName Win32_LogicalDisk | Where-Object { $_.DeviceID -eq "${CDDVMTargetDrive}:" }
                        
                        if (-not $TargetDriveExists) {
                            # Change drive letter to configured target using WMI
                            $CDDrive | Set-CimInstance -Property @{ DriveLetter = "${CDDVMTargetDrive}:" }
...
function Install-VDAFromISO {
    ...
    try {
        ...
        # Standard VDA installer arguments for silent installation
        $VDAArgs = "/quiet /norestart /enable_hdx_ports /enable_real_time_transport /optimize"
...
function Install-PVSFromISO {
    ...
    try {
        ...
        # Standard PVS Target installer arguments for silent installation
        $PVSArgs = "/quiet /norestart"
    ...
    try {
        ...
        if (!$GhostDeviceResults.Success) {
            ...
            try {
                ...
                $AllPnPDevices = Get-CimInstance -ClassName Win32_PnPEntity -ErrorAction SilentlyContinue
...
Export-ModuleMember -Function `
    ...
    'Install-IBMTADDMAgent', 'Configure-IBMTADDMSCMPermissions', 'Install-CitrixOptimizer', 'Optimize-CitrixVDI',
