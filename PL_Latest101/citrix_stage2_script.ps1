    ...
    if ($RequireCacheDrive) {
        ...
        do {
            ...
            if ($Response.ToLower() -eq 'y' -or $Response.ToLower() -eq 'yes') {
                ...
                # Generate HTML report NOW that readiness assessment is complete
                if (-not $ReportGenerated) {
                    Write-Host "`nGenerating comprehensive Stage 2 HTML report with complete readiness data..." -ForegroundColor Cyan
