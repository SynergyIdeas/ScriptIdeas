            ...
            if (![string]::IsNullOrEmpty($LicenseSourcePath)) {
                ...
                if (Test-Path $LicenseSourcePath) {
                    try {
                        ...
                        if (-not $SkipCopy) {
                            if ($LicenseExists) {
                                Remove-Item -Path $LicenseLocalPath -Force -Confirm:$false -ErrorAction SilentlyContinue
                            }
                            Copy-Item -Path $LicenseSourcePath -Destination $LicenseLocalPath -Force -Confirm:$false
                        }
