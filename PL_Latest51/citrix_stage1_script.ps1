...
try {
   ...
   # Pre-VDA installation - only configure spooler service
   Write-LogHeader "Pre-VDA Installation Configuration"
   
   Write-Log "Configuring spooler service before VDA installation..."
   try {
       $SpoolerService = Get-Service -Name "Spooler" -ErrorAction SilentlyContinue
       if ($SpoolerService) {
           if ($SpoolerService.Status -eq "Running") {
               Stop-Service -Name "Spooler" -Force
               Write-Log "Spooler service stopped successfully" "SUCCESS"
           }
           Set-Service -Name "Spooler" -StartupType Disabled
           Write-Log "Spooler service disabled for VDA installation" "SUCCESS"
       } else {
           Write-Log "Spooler service not found" "WARN"
       }
   } catch {
       Write-Log "Failed to configure spooler service: $($_.Exception.Message)" "ERROR"
   }
...
"@
   ...
   try {
       ...
       $ReportPath = Generate-CitrixReport -Stage 1 -InstallResults $InstallConfig.InstallationResults -ValidationResults $FinalValidation -OpenInBrowser $true
