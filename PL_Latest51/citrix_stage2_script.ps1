...
try {
    ...
    Write-Host "`nGenerating Stage 2 HTML report..." -ForegroundColor Cyan
    try {
        ...
        $SystemInfo = @{
            ...
            OSVersion = (Get-WmiOrCimInstance Win32_OperatingSystem).Caption
            TotalMemoryGB = [math]::Round((Get-WmiOrCimInstance Win32_ComputerSystem).TotalPhysicalMemory / 1GB, 1)
        }
        ...
        if ($OptimizationResults.OverallStatus) { $AllValidationResults["System Optimizations"] = $true }
        if ($VDIOptResults.OverallStatus -or $VDIOptResults.Skipped) { $AllValidationResults["VDI Optimizations"] = $true }
        $AllValidationResults["System Readiness"] = ($ReadinessPercentage -ge 70)
        
        # Prepare installation results for Stage 2
        $Stage2Results = @{
            "VDA Verification" = @{ Success = $VDAInstalled }
            "Citrix Services" = @{ Success = ($TotalFoundServices -gt 0); ServiceCount = $TotalFoundServices }
            "System Optimizations" = @{ Success = $OptimizationResults.OverallStatus }
            "VDI Optimizations" = $VDIOptResults
            "System Readiness" = @{ Success = ($ReadinessPercentage -ge 70); Percentage = $ReadinessPercentage }
        }
        
        # Generate and open report
        $ReportPath = Generate-CitrixReport -Stage 2 -InstallResults $Stage2Results -ValidationResults $AllValidationResults -OpenInBrowser $true
    ...
    Write-Host "Interactive HTML report opened in Microsoft Edge" -ForegroundColor Green
    Write-Host ""
