...
try {
    ...
    # Cache all remaining config values before cleanup potentially deletes config file
    Write-Log "Caching final configuration values before cleanup..." "INFO"
    $UseVirtualCacheDrive = [bool](Get-ConfigValue -Key "UseVirtualCacheDrive" -DefaultValue "false" -ConfigFile $ConfigFilePath)
    ...
    # Generate comprehensive HTML report AFTER cache drive operations but BEFORE cleanup deletes scripts
    Write-Host "`nGenerating comprehensive Stage 2 HTML report..." -ForegroundColor Cyan
    try {
        # Import the reporting function while scripts still exist
        ...
        if ($VirtualCacheRemovalResult -and $VirtualCacheRemovalResult.Success) { $AllValidationResults["Virtual Cache Drive Removal"] = $true }
        ...
        $Stage2Results = @{
            ...
            "Virtual Cache Drive Removal" = if ($VirtualCacheRemovalResult) { $VirtualCacheRemovalResult } else { @{ Success = $true; Skipped = $true } }
        ...
        if ($ReportPath) {
            ...
            Write-Host "Report includes all Stage 2 optimizations, cache drive operations, and validations" -ForegroundColor Green
        }
    }
    catch {
