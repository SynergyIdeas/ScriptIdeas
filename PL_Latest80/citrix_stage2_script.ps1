...
# Cache essential config values
$Global:CachedConfig.UseVirtualCacheDrive = [bool](Get-ConfigValue -Key "UseVirtualCacheDrive" -DefaultValue "false" -ConfigFile $ConfigFilePath)
...
try {
    ...
    $RunCitrixOptimizer = $Global:CachedConfig.EnableSystemOptimizations
    ...
    $RequireCacheDrive = $Global:CachedConfig.EnableCacheDriveOperations
    
    # Event Logs Redirection
    $RedirectEventLogsToCache = $Global:CachedConfig.EnableCacheDriveOperations
    ...
    $RedirectUserProfilesToCache = $Global:CachedConfig.EnableCacheDriveOperations
            ...
            if ($Response.ToLower() -eq 'y' -or $Response.ToLower() -eq 'yes') {
                ...
                if (-not $ReportGenerated) {
                    ...
                    try {
                        ...
                        $ReportPath = New-CitrixReport -Stage 2 -InstallResults $Stage2Results -ValidationResults $AllValidationResults -ConfigData $CachedConfig -OutputPath "" -OpenInBrowser $true
                        if ($ReportPath) {
