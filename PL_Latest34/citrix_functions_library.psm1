...
function Set-PageFile {
    ...
    try {
        ...
        try {
            ...
            Write-Log "Registry pagefile management failed: $($_.Exception.Message)" "WARNING"
        }
        ...
        try {
            ...
            Write-Log "Failed to clear existing pagefiles: $($_.Exception.Message)" "WARNING"
        }
...
function New-VirtualCacheDrive {
    ...
    try {
        ...
        $CacheDriveSizeMB = [int](Get-ConfigValue -Key "CacheDriveSizeMB" -DefaultValue $SizeMB -ConfigFile $ConfigFilePath)
        $CacheVHDXPath = Get-ConfigValue -Key "CacheVHDXPath" -DefaultValue "C:\Cache.vhdx" -ConfigFile $ConfigFilePath
        
        # Check if drive letter is already in use
        if (Test-Path "${DriveLetter}:\") {
            Write-Log "Drive ${DriveLetter}: already exists - checking if it's our virtual drive" "WARNING"
        ...
        try {
            ...
            if (Get-Command "New-VHD" -ErrorAction SilentlyContinue) {
                try {
                    New-VHD -Path $CacheVHDXPath -SizeBytes ([int64]$CacheDriveSizeMB * 1MB) -Dynamic
                    ...
                    Format-Volume -DriveLetter $Partition.DriveLetter -FileSystem NTFS -NewFileSystemLabel "Cache" -Force
                    $VHDXCreated = $true
                    Write-Log "VHDX created successfully using Hyper-V cmdlets" "SUCCESS"
                } catch {
                    Write-Log "Hyper-V method failed: $($_.Exception.Message)" "WARNING"
                }
            }
...
"@
                    ...
                    Write-Log "Diskpart method failed: $($_.Exception.Message)" "WARNING"
                }
            }
            
            # Method 3: Fallback to directory-based cache simulation
            if (-not $VHDXCreated) {
                Write-Log "Creating directory-based cache as fallback" "INFO"
                
                # Use C: drive as base for cache directory since D: doesn't exist
