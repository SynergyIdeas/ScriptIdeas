    ...
    switch ($ComponentName) {
        ...
        "NTP Time Configuration" {
            if ($Result.Success) {
                ...
                if ($Result.W32TimeConfigured) {
                    $detailsHTML += '<div class="detail-item detail-success">[SUCCESS] W32Time service configured</div>'
                }
        ...
        "System Optimizations" {
            if ($Result.Success -or $Result.OverallStatus) {
                ...
                if ($Result.OptimizationsApplied -and $Result.OptimizationsApplied.Count -gt 0) {
                    $detailsHTML += "<div class='detail-item detail-success'>[SUCCESS] Applied $($Result.OptimizationsApplied.Count) registry optimizations</div>"
                    $detailsHTML += '<div class="detail-item detail-info">Registry optimizations applied:</div>'
                    foreach ($Optimization in $Result.OptimizationsApplied) {
                        $detailsHTML += "<div class='detail-item detail-success'>  • $Optimization</div>"
                    }
                } else {
                ...
                if ($Result.RegistryKeysModified) {
                    $detailsHTML += "<div class='detail-item detail-success'>Registry keys modified: $($Result.RegistryKeysModified)</div>"
                }
                if ($Result.ServicesOptimized) {
                    $detailsHTML += "<div class='detail-item detail-success'>Services optimized: $($Result.ServicesOptimized)</div>"
                }
                $detailsHTML += '</div>'
            } else {
                $detailsHTML += '<div class="detail-section">'
        ...
        "CitrixOptimizer" {
            if ($Result.Success -or $Result.OverallStatus) {
                ...
                $detailsHTML += "<div class='detail-item detail-warning'>[SKIPPED] Citrix Optimizer disabled in configuration</div>"
                if ($Result.Reason) { $detailsHTML += "<div class='detail-item detail-info'>Reason: $($Result.Reason)</div>" }
                $detailsHTML += '</div>'
            } else {
                $detailsHTML += '<div class="detail-section">'
        ...
        "VDI Optimizations" {
            if ($Result.Success -or $Result.OverallStatus) {
                ...
                if ($Result.RegistryKeysModified -and $Result.RegistryKeysModified.Count -gt 0) {
                    $detailsHTML += "<div class='detail-item detail-success'>[SUCCESS] Modified $($Result.RegistryKeysModified.Count) registry settings</div>"
                    $detailsHTML += '<div class="detail-item detail-info">Registry modifications:</div>'
                    foreach ($RegKey in $Result.RegistryKeysModified) {
                        $detailsHTML += "<div class='detail-item detail-success'>  • $RegKey</div>"
                    }
                } else {
                ...
                if ($Result.OptimizationsCount) {
                    $detailsHTML += "<div class='detail-item detail-success'>Total optimizations: $($Result.OptimizationsCount)</div>"
                }
                $detailsHTML += '</div>'
            } elseif ($Result.Skipped) {
                $detailsHTML += '<div class="detail-section">'
        ...
        "User Profile Redirection" {
            if ($Result.Success) {
                ...
                if ($Result.ProfilePath) {
                    $detailsHTML += "<div class='detail-item detail-success'>[SUCCESS] Profile redirection configured to: $($Result.ProfilePath)</div>"
                } else {
                ...
                if ($Result.RegistryKey) {
                    $detailsHTML += "<div class='detail-item detail-info'>Registry location: $($Result.RegistryKey)</div>"
                }
                if ($Result.ValueName) {
                    $detailsHTML += "<div class='detail-item detail-info'>Registry value: $($Result.ValueName)</div>"
                }
                if ($Result.PreviousValue) {
                    $detailsHTML += "<div class='detail-item detail-info'>Previous value: $($Result.PreviousValue)</div>"
                }
                if ($Result.NewValue) {
                    $detailsHTML += "<div class='detail-item detail-success'>New value: $($Result.NewValue)</div>"
                }
                $detailsHTML += '</div>'
            } elseif ($Result.Skipped) {
                $detailsHTML += '<div class="detail-section">'
        ...
        "Active Setup Components Cleanup" {
            if ($Result.Success) {
                $detailsHTML += '<div class="detail-section">'
                $detailsHTML += '<div class="detail-section-title">Active Setup Components Cleanup</div>'
                if ($Result.TotalRemoved -gt 0) {
                    $detailsHTML += "<div class='detail-item detail-success'>[SUCCESS] Removed $($Result.TotalRemoved) Active Setup components</div>"
                    if ($Result.RemovedComponents -and $Result.RemovedComponents.Count -gt 0) {
                        $detailsHTML += '<div class="detail-item detail-info">Components removed:</div>'
                        foreach ($Component in $Result.RemovedComponents) {
                            $detailsHTML += "<div class='detail-item detail-success'>  • $Component</div>"
                        }
                    }
                } elseif ($Result.Skipped) {
                    $detailsHTML += "<div class='detail-item detail-info'>[SKIPPED] No Active Setup components configured for removal</div>"
                } else {
                    $detailsHTML += "<div class='detail-item detail-info'>[SUCCESS] No problematic components found</div>"
                }
                $detailsHTML += '</div>'
            } else {
                $detailsHTML += '<div class="detail-section">'
