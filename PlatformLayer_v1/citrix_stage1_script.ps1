...
try {
       ...
       if ($ConfigureDNSSuffix) {
           ...
           try {
               ...
               if ($DNSResult) {
                   ...
                   $InstallConfig.InstallationResults.DNSConfiguration = @{ Success = $true; Message = "DNS search list configured" }
               } else {
                   Write-Log "DNS search list configuration failed" "ERROR"
                   $InstallConfig.InstallationResults.DNSConfiguration = @{ Success = $false; Message = "DNS configuration failed" }
               }
           }
           catch {
               Write-Log "Failed to configure DNS search list: $($_.Exception.Message)" "ERROR"
               $InstallConfig.InstallationResults.DNSConfiguration = @{ Success = $false; Message = $_.Exception.Message }
           }
       } else {
           Write-Log "DNS search list configuration disabled in settings" "INFO"
           $InstallConfig.InstallationResults.DNSConfiguration = @{ Skipped = $true; Reason = "Disabled in configuration" }
       }
   ...
   $InstallConfig.InstallationResults.Scripts = @{ Skipped = $true; Reason = "Deferred to Stage 2" }
   $InstallConfig.InstallationResults.ScriptConfiguration = @{ Skipped = $true; Reason = "Deferred to Stage 2" }
