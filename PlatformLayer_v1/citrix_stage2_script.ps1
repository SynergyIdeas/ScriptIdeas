...
try {
    ...
    $AllValidationResults = @{
        ...
        "NTP Time Configuration" = ($NTPConfigResult -and $NTPConfigResult.Success)
        "CitrixOptimizer" = ($OptimizationResults -and $OptimizationResults.OverallStatus)
        ...
        "Run Keys Registry Cleanup" = ($RunKeysResult -and $RunKeysResult.Success)
        ...
        "Virtual Cache Drive Removal" = $false  # Will be updated dynamically later
    }
    ...
    $Stage2Results = @{
        ...
        "System Optimizations" = @{ Success = ($OptimizationResults -and $OptimizationResults.OverallStatus); Details = "Registry optimizations applied" }
        "NTP Time Configuration" = if ($NTPConfigResult) { $NTPConfigResult } else { @{ Success = $false; Skipped = $true } }
        "CitrixOptimizer" = @{ Success = ($OptimizationResults -and $OptimizationResults.OverallStatus); Details = "Citrix Optimizer templates applied" }
        "Scripts" = if ($ScriptCopyResult) { $ScriptCopyResult } else { @{ Success = $false; Skipped = $true; Details = "Script configuration not enabled" } }
        ...
        "Password Age Registry" = @{ Success = $PasswordAgeResult; Details = "Password age cleanup handled by Run Keys function" }
        ...
        "Run Keys Registry Cleanup" = if ($RunKeysResult) { $RunKeysResult } else { @{ Success = $false; TotalRemoved = 0 } }
        ...
        "Virtual Cache Drive Removal" = @{ Success = $false; Details = "Will be updated after cache drive removal" }
    }
