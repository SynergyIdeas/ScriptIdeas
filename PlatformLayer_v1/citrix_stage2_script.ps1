...
# Select OS-specific base template
if ($OSInfo.ScriptSource -eq "win2019") {
    $OSTemplate = "Windows_Server_2019_VDI.xml"
    Write-Log "Selecting Windows Server 2019 base template" "INFO"
} else {
    $OSTemplate = "Windows_Server_2022_VDI.xml"
    Write-Log "Selecting Windows Server 2022 base template" "INFO"
}
...
try {
    ...
    # Initialize Citrix PowerShell environment and disable telemetry
    Write-LogHeader "CITRIX POWERSHELL INITIALIZATION"
    Write-Log "Initializing Citrix PowerShell environment and disabling telemetry..." "INFO"
    
    try {
        $CitrixPSResult = Initialize-CitrixPowerShell
        
        if ($CitrixPSResult.Success) {
            Write-Log "Citrix PowerShell initialization completed successfully" "SUCCESS"
            if ($CitrixPSResult.SnapinsLoaded.Count -gt 0) {
                Write-Log "Loaded PowerShell snapins: $($CitrixPSResult.SnapinsLoaded -join ', ')" "SUCCESS"
            }
            if ($CitrixPSResult.TelemetryDisabled) {
                Write-Log "Citrix Call Home telemetry disabled" "SUCCESS"
            }
            if ($CitrixPSResult.ServiceDisabled) {
                Write-Log "CitrixTelemetryService disabled" "SUCCESS"
            }
            if ($CitrixPSResult.RegistrySet) {
                Write-Log "CEIP telemetry registry disabled" "SUCCESS"
            }
        } else {
            Write-Log "Citrix PowerShell initialization completed with errors" "WARN"
            foreach ($Error in $CitrixPSResult.Errors) {
                Write-Log "  Error: $Error" "ERROR"
            }
        }
    }
    catch {
        Write-Log "Failed to initialize Citrix PowerShell environment: $($_.Exception.Message)" "ERROR"
    }
    ...
    try {
        ...
        if ($UberAgentService) {
            ...
            Write-Log "Configuring UberAgent service for production deployment..." "INFO"
            
            # Set service to Automatic startup for production
            Set-Service -Name $UberAgentServiceName -StartupType Automatic -ErrorAction Stop
            Write-Log "UberAgent service startup type set to Automatic" "SUCCESS"
            Write-Log "Service: $UberAgentServiceName" "INFO"
            Write-Log "Previous startup type: $($UberAgentService.StartType)" "INFO"
            Write-Log "New startup type: Automatic" "INFO"
            ...
            $UberAgentServiceResult = @{ 
                ...
                Message = "UberAgent service startup type set to Automatic"
                Details = "Service configured for automatic startup on production deployment"
            }
        } else {
    ...
    $AllValidationResults = @{
        "VDA Verification" = $VDAInstalled
        "Citrix Services" = ($TotalFoundServices -gt 0)
        "System Optimizations" = ($OptimizationResults -and $OptimizationResults.OverallStatus)
        "VDI Optimizations" = ($VDIOptResults -and ($VDIOptResults.OverallStatus -or $VDIOptResults.Skipped))
        "WEM RSA Cleanup" = $WEMCleanupSuccess
        "Domain Profile Cleanup" = ($ProfileCleanupResults -and $ProfileCleanupResults.Success)
        "VMware Memory Optimization" = ($VMwareMemoryStatus -and $VMwareMemoryStatus.OverallCompliant)
        "Password Age Registry" = $PasswordAgeResult
        "RDS Grace Period Reset" = ($RDSGraceResult -and $RDSGraceResult.Success)
        "Network Optimizations" = ($NetworkOptResults -and $NetworkOptResults.Success)
        "Storage Optimizations" = ($StorageOptResults -and $StorageOptResults.Success)
        "Ghost Device Removal" = ($GhostDeviceResults -and $GhostDeviceResults.Success)
        "System Defragmentation" = ($DefragResults -and $DefragResults.Success)
        "Event Logs Cleanup" = ($EventLogCleanupResults -and $EventLogCleanupResults.Success)
        ".NET Framework Optimization" = ($DotNetResults -and $DotNetResults.Success)
        "Active Setup Components Cleanup" = ($ActiveComponentsResult -and $ActiveComponentsResult.Success)
        "Automatic Maintenance" = ($MaintenanceStatus -and $MaintenanceStatus.Optimized)
        "Recycle Bin Disable" = $RecycleBinResult
        "Quick Access Disable" = $QuickAccessResult
        "Event Log Redirection" = $EventLogResult
        "User Profile Redirection" = $UserProfileResult
        "Pagefile Configuration" = ($PagefileResult -and $PagefileResult.Success)
        "UberAgent Service Configuration" = ($UberAgentServiceResult -and $UberAgentServiceResult.Success)
        "Scripts" = if ($ScriptCopyResult) { $ScriptCopyResult } else { @{ Skipped = $true; Reason = "Not configured" } }
        "ScriptConfiguration" = if ($ScriptConfigResult) { $ScriptConfigResult } else { @{ Skipped = $true; Reason = "Not configured" } }
    }
    ...
    $Stage2Results = @{
        ...
        "Active Setup Components Cleanup" = if ($ActiveComponentsResult) { $ActiveComponentsResult } else { @{ Success = $false; TotalRemoved = 0 } }
        ...
        "UberAgent Service Configuration" = if ($UberAgentServiceResult) { $UberAgentServiceResult } else { @{ Success = $false; Skipped = $true } }
