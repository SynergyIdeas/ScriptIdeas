       ...
       if ($EnableIBMTADDM) {
           ...
           if (Test-Path $TADDMPath) {
               ...
               if ($Process.ExitCode -eq 0) {
                   ...
                   return @{ 
                       Success = $true
                       InstallPath = "C:\IBM\TADDM"
                       Version = "Unknown - check IBM TADDM logs for version details"
                       ExitCode = $Process.ExitCode
                       ...
                       Message = "IBM TADDM agent installation completed successfully"
                   }
               } else {
    ...
    try {
        ...
        if (Test-Path $WEMPath) {
            ...
            if ($WEMProcess.ExitCode -eq 0) {
                ...
                $Results.Success = $true
                $Results.InstallPath = "C:\Program Files (x86)\Norskale\Agent Host"
                ...
                $Results.RegistryChanges = @(
                    "HKLM:\SYSTEM\CurrentControlSet\Control\Norskale\Agent Host created",
                    "HKLM:\SOFTWARE\Norskale\Agent Host configured",
                    "WEM Agent service registration completed"
                )
...
function Add-Domain {
    ...
    try {
        ...
        return @{
            ...
            DomainName = $DomainName
            ComputerName = $env:COMPUTERNAME
            OrganizationalUnit = if ($OUPath) { $OUPath } else { "Default Computers container" }
            RegistryChanges = @(
