        ...
        if ($DDriveInfo) {
            ...
            if ($DDriveInfo.DriveType -eq 5) {
                ...
                try {
                    ...
                    $CDDrive = Get-WmiOrCimInstance -ClassName Win32_Volume | Where-Object { $_.DriveLetter -eq "D:" -and $_.DriveType -eq 5 }
                    ...
                    if ($CDDrive) {
                        ...
                        if (-not $TargetDriveExists) {
                            # Change drive letter to configured target using WMI
                            # Use WMI method for drive letter change (more compatible)
                            $CDDrive.DriveLetter = "${CDDVMTargetDrive}"
                            $CDDrive.Put() | Out-Null
