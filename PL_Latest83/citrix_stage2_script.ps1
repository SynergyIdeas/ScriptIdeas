...
$Global:CachedConfig.CitrixServicesToDisableStage2 = Get-ConfigValue -Key "CitrixServicesToDisableStage2" -DefaultValue "CdfSvc,BITS,Fax,TapiSrv" -ConfigFile $ConfigFilePath
...
if ($UseVirtualCacheDrive) {
    ...
    if ($DDriveExists) {
        ...
        try {
            ...
            $VirtualCacheResult = New-VirtualCacheDrive -VHDXPath $Global:CachedConfig.VirtualCacheDrivePath -SizeMB $Global:CachedConfig.VirtualCacheDriveSizeMB -DriveLetter $Global:CachedConfig.VirtualCacheDriveLetter -Label $Global:CachedConfig.VirtualCacheDriveLabel
...
try {
    ...
    if ($RunCitrixOptimizer) {
        ...
        # Use cached configuration values for Citrix Optimizer
        $CitrixOptimizerResult = Start-CitrixOptimizer -OptimizerPath $Global:CachedConfig.CitrixOptimizerPath -TemplatesPath $Global:CachedConfig.CitrixOptimizerTemplatesPath -OutputPath $Global:CachedConfig.CitrixOptimizerOutputPath
    } else {
    ...
    if ($DisableWindowsServices) {
        ...
        try {
            ...
            $CitrixServicesResult = Stop-CitrixServices -ServiceList $Global:CachedConfig.CitrixServicesToDisableStage2
    ...
    if ($RedirectEventLogsToCache -and $RequireCacheDrive) {
        ...
        try {
            $EventLogResult = Set-EventLogRedirection -CacheDriveLetter $Global:CachedConfig.CacheDriveLetter -EventLogsPath $Global:CachedConfig.EventLogsPath
            if ($EventLogResult) {
    ...
    if ($RequireCacheDrive -and $RedirectUserProfilesToCache) {
        ...
        try {
            $UserProfileResult = Set-UserProfileRedirection -CacheDriveLetter $Global:CachedConfig.CacheDriveLetter -UserProfilesPath $Global:CachedConfig.UserProfilesPath
    ...
    if ($RequireCacheDrive) {
        ...
        $VirtualCacheRemovalResult = Remove-VirtualCacheDrive -VHDXPath $CachedConfig.VirtualCacheDrivePath
