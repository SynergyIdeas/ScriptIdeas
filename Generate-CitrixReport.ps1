        ...
        "Scripts" {
            if ($Result.Success) {
                ...
                if ($Result.Message) {
                    ...
                    $detailsHTML += '<div class="detail-item detail-success">[SUCCESS] Startup and shutdown scripts configured</div>'
                }
                $detailsHTML += '</div>'
                
                # Show registry path configuration
                if ($Result.RegistryPath) {
                    $detailsHTML += '<div class="detail-section">'
                    $detailsHTML += '<div class="detail-section-title">Registry Configuration</div>'
                    $detailsHTML += "<div class='detail-item detail-info'>Registry Path: $($Result.RegistryPath)</div>"
                    if ($Result.PathCreated) {
                        $detailsHTML += "<div class='detail-item detail-success'>Registry path created successfully</div>"
                    } else {
                        $detailsHTML += "<div class='detail-item detail-info'>Registry path already existed</div>"
                    }
                    $detailsHTML += '</div>'
                }
                
                # Show script registration details
                if ($Result.StartupScriptsRegistered -or $Result.ShutdownScriptsRegistered) {
                    $detailsHTML += '<div class="detail-section">'
                    $detailsHTML += '<div class="detail-section-title">Script Registration Results</div>'
                    if ($Result.StartupScriptsRegistered) {
                        $detailsHTML += "<div class='detail-item detail-success'>Startup Scripts: $($Result.StartupScriptsRegistered) registered</div>"
                    }
                    if ($Result.ShutdownScriptsRegistered) {
                        $detailsHTML += "<div class='detail-item detail-success'>Shutdown Scripts: $($Result.ShutdownScriptsRegistered) registered</div>"
                    }
                    $detailsHTML += '</div>'
                }
                
                # Show registered scripts list
                if ($Result.RegisteredScripts -and $Result.RegisteredScripts.Count -gt 0) {
                    $detailsHTML += '<div class="detail-section">'
                    $detailsHTML += '<div class="detail-section-title">Registered Scripts</div>'
                    foreach ($script in $Result.RegisteredScripts) {
                        $detailsHTML += "<div class='detail-item detail-success'>Script: $script</div>"
                    }
                    $detailsHTML += '</div>'
                }
                ...
                $detailsHTML += '<div class="detail-section">'
                $detailsHTML += '<div class="detail-section-title">Scripts Configuration Skipped</div>'
                $detailsHTML += "<div class='detail-item detail-warning'>[SKIPPED] Scripts configuration was skipped</div>"
                if ($Result.Reason) { $detailsHTML += "<div class='detail-item detail-info'>Reason: $($Result.Reason)</div>" }
                $detailsHTML += '</div>'
            } else {
                $detailsHTML += '<div class="detail-section">'
                $detailsHTML += '<div class="detail-section-title">Scripts Configuration Failure</div>'
                $detailsHTML += "<div class='detail-item detail-error'>[ERROR] Scripts configuration failed</div>"
                if ($Result.Error) { $detailsHTML += "<div class='detail-item detail-error'>Error: $($Result.Error)</div>" }
                if ($Result.FailedRegistrations -and $Result.FailedRegistrations.Count -gt 0) {
                    $detailsHTML += '<div class="detail-section">'
                    $detailsHTML += '<div class="detail-section-title">Failed Registrations</div>'
                    foreach ($failure in $Result.FailedRegistrations) {
                        $detailsHTML += "<div class='detail-item detail-error'>Failed: $failure</div>"
                    }
                    $detailsHTML += '</div>'
                }
                $detailsHTML += '</div>'
...
function New-CitrixReport {
    ...
    foreach ($ComponentName in $SortedComponents) {
        ...
        if ($Result -is [hashtable]) {
            ...
            # Check for skip-related error messages (but only if component didn't succeed)
            ...
            # Check for skip-related messages (but only if component didn't succeed)
