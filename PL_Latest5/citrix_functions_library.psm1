    ...
    try {
        ...
        if ($DDriveInfo) {
            ...
            if ($DDriveInfo.DriveType -eq 5) {
                Write-Log "D: drive is a CD/DVD ROM drive - needs to be moved to Z:" "WARN"
                $Results.Issues += "CD/DVD ROM drive is using D: letter - must be changed to Z:"
                $Results.DriveValidationPassed = $false
                
                # Attempt to change CD/DVD ROM drive letter to Z:
                Write-Log "Attempting to change CD/DVD ROM drive letter from D: to Z:..." "INFO"
                try {
                    ...
                    if ($CDDrive) {
                        ...
                        $ZDriveExists = Get-CimInstance -ClassName Win32_LogicalDisk | Where-Object { $_.DeviceID -eq "Z:" }
                        
                        if (-not $ZDriveExists) {
                            # Change drive letter using WMI
                            $CDDrive | Set-CimInstance -Property @{ DriveLetter = "Z:" }
                            Write-Log "Successfully changed CD/DVD ROM drive letter from D: to Z:" "SUCCESS"
                            ...
                            if (-not $DDriveInfoAfter) {
                                ...
                                Write-Host "The CD/DVD ROM drive has been moved from D: to Z:" -ForegroundColor Green
                            ...
                            Write-Log "Z: drive already exists - cannot move CD/DVD ROM drive" "ERROR"
                            $Results.Issues += "Z: drive already in use - cannot relocate CD/DVD ROM drive"
                        }
                    }
                } catch {
...
function Invoke-CitrixOptimizer {
    ...
    try {
        Write-Log "Applying Citrix optimization settings..."
        
        # Basic Citrix optimizations via registry
        $OptimizationKeys = @(
            @{ Path = "HKLM:\SOFTWARE\Citrix\VirtualDesktopAgent"; Name = "ListenPort"; Value = 80 },
        ...
        foreach ($Key in $OptimizationKeys) {
            ...
            Set-ItemProperty -Path $Key.Path -Name $Key.Name -Value $Key.Value -ErrorAction SilentlyContinue
        }
        
        Write-Log "Citrix optimization settings applied" "SUCCESS"
        return $true
    }
    catch {
        Write-Log "Failed to apply Citrix optimizations: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Install-UberAgent {
    ...
    try {
        ...
        if (Test-Path $UberAgentPath) {
            Start-Process -FilePath $UberAgentPath -ArgumentList "/quiet" -Wait
            Write-Log "UberAgent installation completed" "SUCCESS"
            return $true
        }
        else {
            Write-Log "UberAgent installer not found at: $UberAgentPath" "ERROR"
            return $false
        }
    }
    catch {
        Write-Log "Failed to install UberAgent: $($_.Exception.Message)" "ERROR"
        return $false
    }
}
...
function Register-StartupShutdownScripts {
    [CmdletBinding()]
    param(
