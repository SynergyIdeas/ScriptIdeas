...
try {
    ...
    $NetworkSourcePath = Get-ConfigValue "NetworkSourcePath" "\\fileserver\citrix"
    
    # Set configuration variables from config file with NetworkSourcePath-based defaults
    $VDAISOSourcePath = Get-ConfigValue "VDAISOSourcePath" "$NetworkSourcePath\installers\VDA\VDAServerSetup.iso"
    $VDAISOPath = Get-ConfigValue "VDAISOPath" "C:\Temp\VDA.iso"
    $PVSISOSourcePath = Get-ConfigValue "PVSISOSourcePath" "$NetworkSourcePath\installers\PVS\PVS_Target.iso"
...
try {
    ...
    $JoinDomain = Get-ConfigValue "JoinDomain" $false
    if ($JoinDomain) {
        $DomainName = Get-ConfigValue "DomainName" ""
        $OrganizationalUnit = Get-ConfigValue "OrganizationalUnit" ""
    ...
    $Stage2NetworkPath = Get-ConfigValue "Stage2ScriptNetworkPath" "$NetworkSourcePath\scripts\citrix_stage2_script.ps1"
